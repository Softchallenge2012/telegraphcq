 ...
 *
 * $Id: parsenodes.h,v 1.33 2006/02/02 01:46:48 phred Exp $
 *
 *-------------------------------------------------------------------------
 */
#ifndef PARSENODES_H
...
typedef struct Query
{
	...
	Node	*eventClause; /* The event expression from the EVENT clause -SR */
	...
	WindowClause *windowClause; /* a list of WindowExpr's */	/* @dwndwSK */
	AggWindowSpec *aggWinClause;/* a single window specification for
	 * aggregate functions */	/* @dhwndMS */
	...
	/* internal to planner */
	List	   *base_rel_list;	/* list of base-relation RelOptInfo's */
	...
	Oid recurseRelation;        /* Oid of the relation to which tuples for this 
								   query should be fed back */
							
	 
	bool isRecursiveOutput;   /* does this query serve as the output query */


	int			total_machines; /* Machines participating in query */

  CursorType cursorType; /* IStream, RStream, DStream or Unspecified */
} Query;
...
typedef struct AlterStreamStmt
{
	NodeTag		type;
	char		subtype;		/*------------
								 *	W = add wrapper;
								 ...
								 *------------
								 */
	List	   *stream;			/* stream to work on */
	List	   *wrapper;		/* wrapper name */
	List	   *bindings;		/* required input arguments */
  char *info;                           /* stream-wrapper key-value pairs */
}	AlterStreamStmt;

typedef struct Binding
{
	NodeTag		type;
	Node	   *column;		/* required input arguments */
  bool isOptional;
  Value * def;
}	Binding;
...
typedef struct ColumnDef
{
	...
	bool		is_timestamp;	/* TIMESTAMP constraint specified?
								 * @dtstampSK */
...
typedef enum SetOperation
{
	...
	SETOP_INTERSECT,
	SETOP_EXCEPT,
	SETOP_RECURSIVE_NEXT              /*boo hiss: a hack to accept a 
					    set of recursive queries */
} SetOperation;
...
typedef struct SelectStmt
{
	...
	List	   *fromClause;		/* the FROM clause */
        Node       *eventClause;	/* the EVENT clause  -SR */
	Node	   *whereClause;	/* WHERE qualification */
	List	   *groupClause;	/* GROUP BY clauses */
	Node	   *havingClause;	/* HAVING conditional-expression */
	List	   *windowClause;	/* WINDOW clauses */	/* @dwndwSK */
	Node      *resultClause;   /* result clause is for recursive queries */
	AggWindowSpec *aggWinClause;/* WINDOW clause for aggregates */
                                /* @dhwndwMS */
	bool		isContinuousQuery;
	int			total_machines; /* # machines for this query */	/* @dfluxMS */
  ...
  CursorType cursorType; /* Rstream, Istream, Dstream or unspecified */
} SelectStmt;
...
typedef char OverloadType;

/* Parameters for the ON OVERLOAD statement (part of Data Triage) */
typedef struct OverloadParams
{
	OverloadType type;
        /* How to summarize the tuples we don't have time to send to the query
         * processor. */
	char* paramStr;
        /* Options to pass to this particular overload type. */
} OverloadParams;

 /* @BdtcqSK */
/* ------------------------
 * New TelegraphCQ objects
 *
 *		Create Stream Statement
 *
 * Probably will need other fields eventually, so I created another struct which
 * inherits from the previous
 * ------------------------
 */
typedef struct CreateStrmStmt
{
...
/* We would like this to be an enum, but the parser prefers #defines */
/*typedef enum char {*/
#define    OVERLOAD_BLOCK	0
#define    OVERLOAD_DROP	1
#define    OVERLOAD_COUNTS	2
#define    OVERLOAD_REGHIST	3
#define    OVERLOAD_MHIST	4
#define    OVERLOAD_WAVELET	5
#define    OVERLOAD_SAMPLE	6
/*} overload_type;*/

extern OverloadParams* makeOverloadParams(OverloadType type, char* paramStr);

typedef struct CreateWrapperStmt
{
	NodeTag		type;
	char	   *wrappername;
	List	   *functions;
	char	   *info;
}	CreateWrapperStmt;	/* @EdtcqSK */
...
typedef enum ConstrType			/* types of constraints */
{
	CONSTR_NULL,				/* not SQL92, but a lot of people expect
								 * it */
	CONSTR_NOTNULL,
	CONSTR_TIMESTAMPCOLUMN,		/* @dtstampSK */
	CONSTR_DEFAULT,
	CONSTR_CHECK,
	CONSTR_PRIMARY,
	CONSTR_UNIQUE,
...
#define DROP_STREAM   9
#define DROP_WRAPPER  10
...
typedef struct FetchStmt
{
    ...
    bool        iswindow;       /* TRUE if fetching entire windows */
  
  Const *moveInterval; /* if a time movement, how much to move in time */
} FetchStmt;
 ...
 *
 */
typedef struct WindowExpr
{
	NodeTag		 type;
	RangeVar   	*relation;		/* Filled in at parsing time */
	RangeTblRef *rtref;
	Value	   	*rangeby;
	Value      	*slideby;
	List        *slices;
...
/* EventExpr - represents an EVENT expression for event processing
 *
 *            - SR
 */

 typedef struct EventExpr
 {
       NodeTag type;

   struct EventExpr *parent; /* My parent, when arranged in a tree */

       RangeVar *stream_name; /* Makes sense only before the "parsetree" is converted to a "Query" */
       RangeTblEntry *rte; /* Makes sense only after the "parsetree" is converted to a "Query" */
   /* int rt_index; */
   Var *var;

       List       *partition;    /* For the PARTITION BY clause */  
   List *var_lists; /* A complicated list of lists, when partition gets resolved */

   /* Currently, we have only upto-ternary event operators */
   EventOperator event_op;
   struct EventExpr *lchild;
   struct EventExpr *rchild;
   Value *time_delay_val;
   Interval time_delay;

   List *outputList; /* List of events generated by this operator... alternatively, input list for the parent operator */
