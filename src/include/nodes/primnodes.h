 ...
 *
 * $Id: primnodes.h,v 1.14 2004/07/15 20:39:58 owenc Exp $
 *
 *-------------------------------------------------------------------------
 */
#ifndef PRIMNODES_H
...
typedef struct RangeVar
{
  ...
  Node *window;                          /* holds the window specification to go along with this rangevar */    
} RangeVar;
...
/*																	 //@BdwndwSK
 * The TSNATTR is the "special" marker for the virtual Tuple Sequence Number
 * attribute in an IntermediateHeapTuple
 */
#define TSNATTR 1023
 /* @EdwndwSK */
typedef enum OptrType			/* @BdcacqSK */
{
	LT_OPTR, LE_OPTR, GT_OPTR, GE_OPTR, EQ_OPTR, NE_OPTR, OTHER_OPTR
}	OptrType;			/* @BdcacqSK */
...
typedef struct Oper
{
	...
	OptrType	optrtype;		/* Indicates if operator is special
								 * @dcacqSK */
} Oper;
...
/*----------														 //@BdcacqSK
 * ConstTree - An in-memory binary tree
 *
 * This is used to represent collections of predicates in a Grouped Selection
 * Filter of a CQEddy
 *
 *----------
 */
typedef struct ConstTree
{
	NodeTag		type;
	Const	   *c;
	List	   *queries;
	List	   *eqqueries;
	struct ConstTree *left;
	struct ConstTree *right;
}	ConstTree;			/* @EdcacqSK */

/*----------														   @BdwndwSK
 * WindowClause - represents a WINDOW ... construct
 *
 *----------
 */
...
{
	NodeTag		type;
	List	   *windowExprs;	/* List of WindowExpr */
	Node	   *max;			/* max of each window var */
}	WindowClause;		/* @EdwndwSK */


/*----------
 * CursorType - the type of stream the client is trying to extract
 *----------
 */
typedef enum CursorType
{
  CURSORKIND_UNKNOWN = 0,
  CURSORKIND_ISTREAM,
  CURSORKIND_RSTREAM,
  CURSORKIND_DSTREAM
} CursorType;

#define StringOfCursorType(type) \
(((type) == CURSORKIND_ISTREAM) ? "IStream" : \
 ((type) == CURSORKIND_RSTREAM) ? "RStream" : \
 ((type) == CURSORKIND_DSTREAM) ? "DStream" : \
 "Unspecified")
