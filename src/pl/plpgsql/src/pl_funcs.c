 ...
 *
 * IDENTIFICATION
 *	  $Header: /project/eecs/db/cvsroot/postgres/src/pl/plpgsql/src/pl_funcs.c,v 1.3 2006/02/06 01:19:46 phred Exp $
 *
	...
	while (*s)
	{
		...
		*cp = '\0';
		...
		if (i >= NAMEDATALEN)
		{
			...
			len = pg_mbcliplen((unsigned char *)curident, i, NAMEDATALEN - 1);
			curident[len] = '\0';
		}
