...
/* $Header: /project/eecs/db/cvsroot/postgres/src/interfaces/jdbc/org/postgresql/jdbc1/AbstractJdbc1Statement.java,v 1.3 2004/03/24 08:11:32 chungwu Exp $
...
{
   // utility: an object[0] we need sometimes
   ...
   {
      m_fetchSize=((AbstractJdbc1Connection) connection).defaultFetchSize;
   }
   ...
   protected void resetSqlFragments() throws SQLException
   {
   ...
   /** small utility routine used for setting the m_sqlFragments
    *  property to a new value. Due to the use of prepared statements
    *  declared at the backend, we should deallocate these if we
    *  get a new query. This is done in here automatically.
    */
   protected void setSqlFragments(String s) throws SQLException
   {
      resetSqlFragments();
      String l_sql = replaceProcessing(s);
   ...
   /*
    * The "real" worker behind all execute*(*) methods.
    * @param update tip if we are NOT expecting a big result set
    *     (so we won't use cursor for it)
    * @return true if the next result is a ResultSet; false if it is an
    *     update count or no result at all.
    * @exception SQLException if a database access error occurs
    */
   protected boolean execute(boolean update) throws SQLException
   {
		Driver.debug("execute called");
      ...
      // cursors dont work in autocommit mode
      // fetchsize must be set >0
      // we should be aware of a result set (so no update calls or somewhat like that.
      boolean use_cursor=true;
      if ((      m_fetchSize<1) || update || (((AbstractJdbc1Connection)connection).autoCommit))
	  {
	      use_cursor=false;
	      Driver.debug("setting use_cursor to false");
	      Driver.debug("m_fetchSize=" + m_fetchSize + ", update=" + update + ", autocommit = " + ((AbstractJdbc1Connection)connection).autoCommit);
	  }
	Driver.debug("useCursor = " + use_cursor);
      String[] frags;
		//Use server prepared statements if directed
      if (m_useServerPrepare && !use_cursor) // we have to change that as soon as cursors AND preparing work (7.4?)
		{
         if (m_statementName == null) // statement is not declared till now, so let's declare
			{
				...
				synchronized (sbuf)
				{
					...
					sbuf.append("; ");
               /* we have to wait for 7.4 to use this :-(
               // in case of cursor usage, prepend cursor declaration string
               if (use_cursor)
                  sbuf.append(getBeginCursorDeclString(m_executeSqlFragments[0]));
               else
               */
					sbuf.append(m_executeSqlFragments[0]);

					m_sqlFragments[0] = sbuf.toString();
               int l=m_sqlFragments.length-1;
               System.arraycopy(m_executeSqlFragments, 1, m_sqlFragments, 1, l);
               /* we have to wait for 7.4 to use this :-(
               // if cursor is used, we modify the "tail" on m_sqlFragments
               if (use_cursor)
                  m_sqlFragments[l]=getEndCursorDeclString(m_sqlFragments[l]);
               */
               frags=m_sqlFragments;
				}

			}
         else // already declared, to call it ("execute..."), just use executeSQLFragments
			{
				m_sqlFragments = m_executeSqlFragments;
            /* see above. 7.4. sigh.
            if (use_cursor)
               frags=getCursoredSqlFrags(m_sqlFragments);
            else // no cursor
            */
               frags=m_sqlFragments;
			}
		}
      else // no server prepare at all. Maybe cursor?
      {
         if (use_cursor)
            frags=getCursoredSqlFrags(m_sqlFragments);
         else //  no cursor
	...
	Driver.debug("frags length=" + frags.length);
      if(connection.useTelegraphCQ == false)
	  {
	      ((AbstractJdbc1Connection)connection).execSQL(frags, m_binds,
      ...
      else
	  {
	      ...
	      if((use_cursor == true && frags.length != 2) ||
		 (use_cursor == false && frags.length != 1)) 
		  {
		      throw new SQLException("TelegraphCQ does not support query batches");
		  }
	      
	      sql[0] = frags[0];
	      ((AbstractJdbc1Connection)connection).execSQL(sql, m_binds,
							    (java.sql.Statement)this, result);
	      sql[0] = frags[1];
	      ((AbstractJdbc1Connection)connection).execSQL(sql, m_binds,
							    (java.sql.Statement)this, result);
   ...
   public String[] getCursoredSqlFrags(String[] org_frags)
   {
      int l=org_frags.length;
      String[] frags=new String[l+1];
      System.arraycopy(org_frags,0,frags,0,l);
      frags[l]="";
      if (l>0)
      {
         frags[0]=getBeginCursorDeclString(frags[0]);
         frags[l]=getEndCursorDeclString(frags[l]);
      }
      return frags;
   }

   public String getBeginCursorDeclString(String first)
   {
      m_cursorName= "JDBC_CURS_"+m_preparedCount++;
      return new StringBuffer("DECLARE ")
                  .append(m_cursorName)
                  .append(" CURSOR FOR ")
                  .append(first)
                  .toString();
   ...
   public String getEndCursorDeclString(String last)
   {
      return new StringBuffer(last)
	...
	public void close() throws SQLException
	{
		...
		result = null;
      // if any server side prepared statements were created, dealloc them.
      resetSqlFragments();
	...
	public void setLong(int parameterIndex, long x) throws SQLException
	{
      ...
      // fix for slow index lookup on non-explicit cast to bigint.
      //bind(parameterIndex, Long.toString(x)+"::bigint", PG_TEXT);
	}
	...
	public void setObject(int parameterIndex, Object x, int targetSqlType, int scale) throws SQLException
	{
		...
		switch (targetSqlType)
		{
			case Types.INTEGER:
				...
				break;
         case Types.BIGINT:
            bind(parameterIndex, x.toString(), PG_INT8);
            //fix for bad index lookup performance in bigint-case. Explicit cast helps here...
            //bind(parameterIndex, x.toString()+"::bigint", PG_TEXT);
            break;
			case Types.TINYINT:
			case Types.SMALLINT:
			case Types.REAL:
			...
			case Types.BIT:
				if (x instanceof Boolean)
				{
               bind(parameterIndex, ((Boolean)x).booleanValue() ? "TRUE" : "FALSE", PG_BOOLEAN);
				}
				else
				{
    ...
    public void setUseServerPrepare(boolean flag) throws SQLException {
      if (m_useServerPrepare == flag) // no change?
         return ;
