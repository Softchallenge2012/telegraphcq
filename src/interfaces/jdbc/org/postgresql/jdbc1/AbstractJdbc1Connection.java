...
/* $Header: /project/eecs/db/cvsroot/postgres/src/interfaces/jdbc/org/postgresql/jdbc1/AbstractJdbc1Connection.java,v 1.3 2004/03/24 08:11:31 chungwu Exp $
...
public abstract class AbstractJdbc1Connection implements org.postgresql.PGConnection
{
    ...
    public boolean useTelegraphCQ=false;
   ...
   // some defaults for statements
   public int defaultFetchSize = 0;       // do not fetch by default. Use defaultFetchSize=xx in URL to change.
   public boolean forceFetchSize = false;  // do not allow programmatic fetch size alteration. Use forceFetchSize in URL to change.
   public boolean explicitCloseCursors = false; // explicitly close cursors on ResultSet close (needs another roundtrip).
   // These two properties make a conjunction of the preparing policy:
   // ALL means ..Always to true, never means ...Never to true.
   // This is the fastest way to distinguish if or if not to prepare.
   public boolean useServerPrepareAlways = false;
   public boolean useServerPrepareNever = false;
	/*
	...
	public void openConnection(String host, int port, Properties info, String database, String url, org.postgresql.Driver d) throws SQLException
	{
      ...
      // Apply defaults. We have to do that at that rather late point to get
      // information about version of backend etc.
      // Set default Statement parameters
      if (info.containsKey("defaultFetchSize"))
      {
         defaultFetchSize=new Integer(info.getProperty("defaultFetchSize")).intValue();
      }
      if (info.containsKey("forceFetchSize"))
      {
         forceFetchSize=new Boolean(info.getProperty("forceFetchSize")).booleanValue();
      }
      if (info.containsKey("explicitCloseCursors"))
      {
         explicitCloseCursors=new Boolean(info.getProperty("explicitCloseCursors")).booleanValue();
      }
      if (info.containsKey("useServerPrepare"))
      {
         if (haveMinimumServerVersion("7.3")) { // only >=7.3 can do this.
            String s=info.getProperty("useServerPrepare").toLowerCase();
            if (s.equals("always"))
               useServerPrepareAlways=true;
            else if (s.equals("never"))
               useServerPrepareNever=true;
         }
         else // <7.3 servers aren't capable of preparing at all.
         {
            useServerPrepareNever=true;
         }
	}

      if(org.postgresql.Driver.useTelegraphCQ(info))
	  {
	      useTelegraphCQ=true;
		if(defaultFetchSize > 0 ) 
		{
	     	 setAutoCommit(false); 
	      	Driver.debug("setting autocommit to false ");
		}
	  }
   }
   ...
   /** Simple query execution.
	 */
   public java.sql.ResultSet doQuery (String s, java.sql.Statement stat, java.sql.ResultSet rs) throws SQLException
	{
      execSQL(new String[] { s }, nullarr, stat, rs);
      return rs;
	}

   public java.sql.ResultSet doQuery (String s, java.sql.Statement stat) throws SQLException
		{
      java.sql.ResultSet rs = createResultSet(stat);
      return doQuery(s,stat,rs);
		}
   ...
   public void execSQL(String[] p_sqlFragments,
	...
	for(int i=0; i < p_sqlFragments.length; i++)
		Driver.debug("execSQL " + i + ":" + p_sqlFragments[i]);
	...
	public void setAutoCommit(boolean autoCommit) throws SQLException
	{
		...
		if (autoCommit)
		{
			if (haveMinimumServerVersion("7.3"))
			{
				...
				doQuery("select 1;");
				doQuery("commit; set autocommit = on;");
			}
			else
			{
				doQuery("end");				
