...
/* $Header: /project/eecs/db/cvsroot/postgres/src/interfaces/jdbc/org/postgresql/jdbc1/AbstractJdbc1ResultSet.java,v 1.3 2004/03/24 08:11:32 chungwu Exp $
...
public abstract class AbstractJdbc1ResultSet implements org.postgresql.PGResultSet
{
   ...
   {
      return (((AbstractJdbc1Statement)statement).m_cursorName!=null);
   }

   public String getCursorName()
   {
      return (((AbstractJdbc1Statement)statement).m_cursorName);
   }

   public void closeCursor() throws SQLException
   {
      // do we really need this? It's another roundtrip to the database...
      // if one thinks it needed, then here's the way to add it.
      /*
      if (isCursored())
      {
         ((AbstractJdbc1Connection)connection).doQuery(
            new StringBuffer("CLOSE ").append(getCursorName()).toString());
         ((AbstractJdbc1Statement)statement).m_cursorName=null;
      }
      */
   }
   ...
   {
      return (((AbstractJdbc1Statement)statement).m_fetchSize);
   }
   ...
   // This slightly altered by nic.
	public boolean next() throws SQLException
	{
		...
		if (++current_row >= rows.size())
      {
         if (!isCursored())
         {
            return false; // we do not fetch by cursor, so this is the end...
         }
         // Smart optimization: If the rows.size() is smaller than the
         // last fetch did end by not filling up completely -> we are at the end.
         // So we do not need another roundtrip to the database.
         if (rows.size()<getFetchSize())
         {
            return false;
         }
         ...
         ((AbstractJdbc1Connection)connection).doQuery(
            new StringBuffer("FETCH FORWARD ").append(getFetchSize())
                  .append(" FROM ").append(getCursorName()).toString(),
            statement, (java.sql.ResultSet)this);
         if (rows.size() == 0)
         {
            // rows are still empty. So no data anymore...
			return false;
         }

         // New data arrived. Reset the counter and let it go on...
         current_row = 0;
      }
	...
	public void close() throws SQLException
	{
      ...
      // if it was cursored, we should the server side cursor as well.
      // Note: this RS shouldn't be used after this call!
      closeCursor();
	}
