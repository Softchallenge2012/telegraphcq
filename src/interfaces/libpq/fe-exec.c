 ...
 *
 * IDENTIFICATION
 *	  $Header: /project/eecs/db/cvsroot/postgres/src/interfaces/libpq/fe-exec.c,v 1.3 2004/05/15 18:57:34 chungwu Exp $
 *
 *-------------------------------------------------------------------------
 */
		...
		else
		{
			...
			switch (id)
			{
				...
				case 'H':		/* Start Copy Out */
			        ...
			        case 'W':
				  /* Window is dry! */
				  conn->result->resultStatus = PGRES_WINDOW_DRY;
					break;
				default:
					printfPQExpBuffer(&conn->errorMessage,
									  libpq_gettext(
...
getAnotherTuple(PGconn *conn, int binary)
{
	...
	if (result->resultStatus == PGRES_WINDOW_DRY) {
	  /* If the window was previously dry, it is no longer dry since
	   * we just found some tuples.  Set the result status back to
	   * happy land.
	   */
