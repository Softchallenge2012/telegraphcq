#! /bin/sh
# $Header: /project/eecs/db/cvsroot/postgres/src/test/regress/pg_regress.sh,v 1.4 2003/04/29 21:25:13 mashah Exp $
...
then
    ...
    postmaster_options="$postmaster_options "
    ...
    echo "postmaster_options=$postmaster_options"
    i=0
    max=60
...
if [ "$CQOPTS" == "true" ]; then
    echo "Running in CQ MODE... restarting postmaster"
    # ----------
    # Start postmaster
    # ----------
    ...
    sleep 10;
    message "starting postmaster"
    postmaster_options="$postmaster_options -G -t $dbname -u regressuser1 "
    [ "$debug" = yes ] && postmaster_options="$postmaster_options -d 5"
    [ "$unix_sockets" = no ] && postmaster_options="$postmaster_options -i"
    "$bindir/postmaster" -D "$PGDATA" -F $postmaster_options >"$LOGDIR/postmaster.log" 2>&1 &
    postmaster_pid=$!
