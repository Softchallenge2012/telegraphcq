...
$usage = "ss.pl host port sourcename duration [-fs sensor_freq]
 where
...
while ($dur >= 0 || $forever == 1) {
	    ...
	    system("wget -q http://www.dot.ca.gov/traffic/d4/update.txt -O scratch/update.txt");
	    system("gawk -f add_time.awk scratch/update.txt > scratch/sensor-counts");
		system("cat scratch/sensor-counts >> scratch/sensor-counts-log");
	    print ("Done.\n");
	    print ("Sending data to TelegraphCQ.\n");
		open(SENSOR, "scratch/sensor-counts") || die "Can't open sensor data file: $!\n";
	    $count=0;
		while (<SENSOR>) {
		  #parse the line
