...
// $Id: HttpBasicServer.java,v 1.2 2003/07/17 05:19:13 sailesh Exp $
...
public class HttpBasicServer extends HttpServer {
	...
	for (int loop = 0 ; loop < addrs.length ; loop++) {
	    ...
	    try {
			if(isHttps == false)
				...
				{
					conn = new HttpsBasicConnection(this
												   , connid++
												   , addr
												   , port
												   , timeout
    ...
    public void initialize(HttpManager manager
			   ...
			   , String host, int port, int timeout, boolean isHttps) 
	throws HttpException
    {
	...
	System.out.println("registering a new server for " + host + "port" + isHttps);
	// Get this server IP addreses:
	...
	try {
		...
		if (request.hasOutputStream()) {
		    ...
		    if (length != -1) {
			request.getOutputStream().mark(length+1);
		    } else { // FIXME fixed length for unknown stream length
			request.getOutputStream().mark(1024*64); 
		    }
		}
		try {
