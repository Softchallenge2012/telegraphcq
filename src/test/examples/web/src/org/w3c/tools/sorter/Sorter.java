// Sorter.java
// $Id: Sorter.java,v 1.2 2003/07/17 05:19:10 sailesh Exp $
...
public class Sorter {
    ...
    static void orderedComparableInsert(org.w3c.tools.sorter.Comparable key, Vector into) {
	int lo  = 0 ;
	int hi  = into.size() - 1 ;
	int idx = -1 ;
	org.w3c.tools.sorter.Comparable item = null ;
	int cmp = 0 ;

	if ( hi >= lo ) {
	    while ((hi - lo) > 1) {
		idx  = (hi-lo) / 2 + lo ;
		item = (org.w3c.tools.sorter.Comparable) into.elementAt(idx) ;
	    ...
	    switch (hi-lo) {
	      case 0:
		item = (org.w3c.tools.sorter.Comparable) into.elementAt(hi) ;
		idx = (item.greaterThan(key)) ? hi : hi+1;
		break ;
	      case 1:
		org.w3c.tools.sorter.Comparable loitem = (org.w3c.tools.sorter.Comparable) into.elementAt(lo) ;
		Comparable hiitem = (org.w3c.tools.sorter.Comparable) into.elementAt(hi) ;
    ...
    public static Vector sortComparableEnumeration(Enumeration enum) {
	Vector sorted = new Vector() ;
	while ( enum.hasMoreElements() ) 
	    orderedComparableInsert((org.w3c.tools.sorter.Comparable) enum.nextElement(), sorted) ;
	sorted.trimToSize();
	return sorted ;
    }
    ...
    static void quickSortCompArray(org.w3c.tools.sorter.Comparable array[], int lo0, int hi0) {
	int lo = lo0 ;
	int hi = hi0 ;
	org.w3c.tools.sorter.Comparable mid = null ;
	...
	if ( hi0 > lo0 ) {
		...
		while ((hi > lo0) && (array[hi].greaterThan(mid)))
		    --hi ;
		if ( lo <= hi ) {
		    org.w3c.tools.sorter.Comparable tmp = array[lo] ;
		    array[lo]  = array[hi] ;
		    array[hi]  = tmp ;
		    ++lo ;
		    --hi ;
    ...
    public static org.w3c.tools.sorter.Comparable[] sortComparableArray(org.w3c.tools.sorter.Comparable array[],
						   boolean inplace) 
    {
	org.w3c.tools.sorter.Comparable tosort[] =array;
	if ( ! inplace ) {
	    tosort = new org.w3c.tools.sorter.Comparable[array.length] ;
	    System.arraycopy(array, 0, tosort, 0, array.length) ;
	}
