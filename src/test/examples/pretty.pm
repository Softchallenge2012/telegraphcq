...
# First argument is the message, second is the number
# of seconds to delay.
sub please_wait($$) {
    my ($msg, $seconds) = @_;

    my $PLEASE_WAIT_INTERVAL_SEC = 0.1;
    my $PLEASE_WAIT_INTERVAL_USEC = int(1e6 * $PLEASE_WAIT_INTERVAL_SEC);

    my $num_steps = int($seconds / $PLEASE_WAIT_INTERVAL_SEC);

    for (my $i = 0; $i < $num_steps; $i++) {
        status_graph($msg, $i, $num_steps-1);
        usleep($PLEASE_WAIT_INTERVAL_USEC);
    }
