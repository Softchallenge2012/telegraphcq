 ...
 *
 * IDENTIFICATION
 *	  $Header: /project/eecs/db/cvsroot/postgres/src/backend/utils/adt/timestamp.c,v 1.5 2004/03/24 08:11:13 chungwu Exp $
 *
 *-------------------------------------------------------------------------
 */
...
AdjustTimestampForTypmod(Timestamp *time, int32 typmod)
{
		...
		&& (typmod != -1) && (typmod != MAX_TIMESTAMP_PRECISION))
	{
		 ...
		 * round-to-nearest, but the integer code always rounds up (away
		 * from zero).	Is it worth trying to be consistent?
		 */
#ifdef HAVE_INT64_TIMESTAMP
...
/*			interval->time = (int)(interval->time); */
		...
		if (precision != INTERVAL_FULL_PRECISION)
		{
			 ...
			 * rounds up (away from zero).	Is it worth trying to be
			 * consistent?
			 */
#ifdef HAVE_INT64_TIMESTAMP
 ...
 *
 * Converts an interval to the appropriate span of time in sec. or usec.
 */
#ifdef HAVE_INT64_TIMESTAMP
    int64
#else
    double
#endif
interval_to_timespan(Interval *i) 
{
#ifdef HAVE_INT64_TIMESTAMP
	int64		span;
#else
...
#ifdef HAVE_INT64_TIMESTAMP
	if (i->month != 0)
		span += ((i->month * INT64CONST(30) * INT64CONST(86400000000)));
#else
	if (i->month != 0)
		span += (i->month * (30.0 * 86400));
#endif
...
interval_cmp_internal(Interval *interval1, Interval *interval2)
{
	...
	span1 = interval_to_timespan(interval1);
	span2 = interval_to_timespan(interval2);
...
/* Division; returns <interval1> / <interval2>.  The units cancel out, and
 * we're left with a ratio, which we return as a double. */
Datum
interval_div_interval(PG_FUNCTION_ARGS)
{
    Interval    *span1 = PG_GETARG_INTERVAL_P(0);
    Interval    *span2 = PG_GETARG_INTERVAL_P(1);
    float8       result = -1;

    result = (float8)interval_to_timespan(span1) 
                / (float8)interval_to_timespan(span2);
