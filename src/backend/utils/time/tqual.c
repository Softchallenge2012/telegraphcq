 ...
 *
 * IDENTIFICATION
 *	  $Header: /project/eecs/db/cvsroot/postgres/src/backend/utils/time/tqual.c,v 1.9 2004/02/18 02:23:49 phred Exp $
 *
 *-------------------------------------------------------------------------
 */
...
{
	/*
	 * SIRISH this is needed to make sure tuples are visible to *//* @Bdtcq
	 * SK
	 */
	/* future queries after heap_insert in the streamer */

	/*
	 * if(MyProcessType == PROC_TCQ_BACKEND || MyProcessType ==
	 * PROC_TCQ_WRAPPER)
	 */
	/* return true; */
	if (AMI_OVERRIDE)
