 ...
 *
 * IDENTIFICATION
 *	  $Header: /project/eecs/db/cvsroot/postgres/src/backend/utils/mmgr/mcxt.c,v 1.12 2005/06/13 02:59:28 phred Exp $
 *
 *-------------------------------------------------------------------------
 */
...
/* TelegraphCQ memory contexts.  The Shared memory context is a
   copy of the memory context routimes will locking and calls to
   malloc and friends replaced with calls to the OSSP MM shared
   memory library.	The TopSharedMemoryContext must be initilized
...
/*
  telegraphCQ creates a subcontext of the TopSharedMemoryContext for each
   query that is passed between the TelegraphCQ frontend and backend.  Memory
   ownership is passed between processes by handing off this context.
*/
...
void MemoryContextTraceAlloc(MemoryContext context, Size size, void *ptr);
...
MemoryContextReset(MemoryContext context)
{
	...
	if(enable_memory_debugging && tcqbackendinitialized && *tcqbackendinitialized == true && *tcqwrapchinitialized == true)
	  {
	    elog(LOG, "%d: Memory context reset called on context '%s'", 
                getpid(),
                context->name);
	  }
...
MemoryContextDelete(MemoryContext context)
{
	...
	if(ShouldTrace()) 
	  {
	    elog(LOG, "%d: Memory context delete called on context '%s'", 
                getpid(), context->name);
	  }
					...
					const char *name)
{
	...
	if(ShouldTrace()) 
	  {
	    elog(LOG, "%d: Memory context '%s' with size %lu created", 
                getpid(), name, (unsigned long)size);
	  }
	return node;
}
...
MemoryContextAlloc(MemoryContext context, Size size)
{
  void *ptr=NULL;
    ...
    ptr = (*context->methods->alloc) (context, size);
    if(ShouldTrace()) 
    {
        MemoryContextTraceAlloc(context, size, ptr);
    }
    return ptr;
}
...
pfree(void *pointer)
{
	...
	AssertArg(MemoryContextIsValid(header->context));
	 ...
	 if(ShouldTrace())
	  {
	    elog(LOG,
                "%d: freeing %p of size %lu in context '%s'", 
                getpid(),
                pointer, (unsigned long)header->size, header->context->name); 
	  }
...
/*******************************************************************************
 * MemoryContextTraceAlloc
 *
 * Should be called from MemoryContextAlloc; sends a trace message to the
 * logfile.
 ******************************************************************************/
void MemoryContextTraceAlloc(MemoryContext context, Size size, void *ptr) {
    /*bool print = true;*/
    /* Cut out initialization of static globals, as well as front-end
     * processes and old-fashioned Postgres. */
    /*
    if (MyProcessType != PROC_TCQ_BACKEND) {
        print = false;
    }
    if (MyProcessType == PROC_TCQ_BACKEND
            && (!tcqbackendinitialized 
    ...
    */
        ...
        /* Code copied from TraceDump(), which in turn uses the GNU C library
         * */
        stack_size = backtrace(stack, 100);

        /* Print out the entire stack trace, skipping internal functions. */
        for (i = 2; i < stack_size; i++) {
            int nprinted = snprintf(buf + len, sizeof(buf) - len, "%p,",
                    stack[i]);
            len += nprinted;
            Assert(len > 0 && len < sizeof(buf));
        }
        /* Turn the extra comma at the end of the string into a trailing NULL */
        buf[len-1] = '\0';

        elog(LOG, 
            "%d: allocating %lu bytes at %p in context '%s' from %s",
                getpid(), (unsigned long)size, ptr, context->name, buf);
    /*}*/

}

/* Returns true if we should generate a trace message at this point. */
bool ShouldTrace(void) {
