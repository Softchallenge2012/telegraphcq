 ...
 *
 * IDENTIFICATION
 *	  $Header: /project/eecs/db/cvsroot/postgres/src/backend/utils/error/elog.c,v 1.12 2004/03/24 08:11:14 chungwu Exp $
 *
 *-------------------------------------------------------------------------
 */
#include "postgres.h"
	...
	if (lev == ERROR || lev == FATAL)
	{
		...
		if (lev == FATAL || !Warn_restart_ready || proc_exit_inprogress)
		{
			...
			if (IsUnderPostmaster &&
				(MyProcessType == PROC_TCQ_BACKEND ||
				 MyProcessType == PROC_TCQ_WRAP_CH))
			{
				/* unless we've explicitly redirected the jump */
				if (elog_jump_buf == NULL ||
					elog_jump_buf == &Warn_restart)
					proc_exit(1);
...
print_pid(void)
{
	...
	snprintf(buf, PID_SIZE + 1, "[%5d]      ", (int) MyProcPid);
	return buf;
}
