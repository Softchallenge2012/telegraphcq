 ...
 *
 * IDENTIFICATION
 *	  $Header: /project/eecs/db/cvsroot/postgres/src/backend/utils/hash/dynahash.c,v 1.2 2004/03/24 08:11:14 chungwu Exp $
 *
 *-------------------------------------------------------------------------
 */
/*
...
hash_create(const char *tabname, long nelem, HASHCTL *info, int flags)
{
	...
	else
	{
		if (flags & HASH_CONTEXT)
		{
		    ...
		    if(info->hcxt == TopSharedMemoryContext)
			{
				CurrentDynaHashCxt = SHMAllocSetContextCreate(info->hcxt,
						"DynaHashTable",
						ALLOCSET_DEFAULT_MINSIZE,
						ALLOCSET_DEFAULT_INITSIZE,
						ALLOCSET_DEFAULT_MAXSIZE,
