 ...
 *
 * IDENTIFICATION
 *	  $Header: /project/eecs/db/cvsroot/postgres/src/backend/nodes/print.c,v 1.12 2004/03/24 08:10:57 chungwu Exp $
 *
 * HISTORY
...
print_slot(TupleTableSlot *slot)
{
	...
	{
		printf("The slot contains an intermediate tuple\n");
		printf("+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++\n");
		debugitup((IntermediateHeapTuple) slot->val);
		printf("-------------------------------------------------------------\n");
	}
}
...
/* @BdfluxMS */
void
f_print_slot(TupleTableSlot *slot, FILE *fp)
{
	if (!slot->val)
	{
		fprintf(fp, "tuple is null.\n");
		return;
	}
	...
	if (!slot->ttc_isIntermediateTuple)
	{
		if (!slot->ttc_tupleDescriptor)
		{
			fprintf(fp, "no tuple descriptor.\n");
			return;
		}

		f_debugtup(slot->val, slot->ttc_tupleDescriptor, NULL, fp);
	}
	else
	{
		fprintf(fp, "The slot contains an intermediate tuple\n");
		fprintf(fp, "+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++\n");
	}
