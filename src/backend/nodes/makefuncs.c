 ...
 *
 * IDENTIFICATION
 *	  $Header: /project/eecs/db/cvsroot/postgres/src/backend/nodes/makefuncs.c,v 1.9 2004/03/24 08:10:57 chungwu Exp $
 */
#include "postgres.h"
...
/*																	   @BdcacqSK
 * makeConstTree -
 *	  creates a VarTree node
 */
ConstTree *
makeConstTree(Const *c)
{
	ConstTree  *vt = makeNode(ConstTree);

	vt->c = c;
	vt->queries = NIL;
	vt->eqqueries = NIL;
	vt->left = vt->right = NULL;
	return vt;
}	/* @EdcacqSK */
