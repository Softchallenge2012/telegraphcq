 ...
 *
 * IDENTIFICATION
 *	  $Header: /project/eecs/db/cvsroot/postgres/src/backend/nodes/list.c,v 1.10 2005/11/25 20:09:23 rizvi Exp $
 *
 * NOTES
...
/*
 * like ptrMember(), but returns the position of l1 in the list l2 if found and
   -1 otherwise
 */
int
ptrMemberWhich(void *l1, List *l2)
{
	List	   *i;
	int			which = 0;
 ...
 *	  See also LispRemove.
 * 
 * This new default version of lremove() will free the "stub" (the list node)
 * that points to the element being deleted
 * Although this looks harmless (as no one is expected to maintain a pointer to that,
 * and we are also not returning a pointer to the removed stub), we found a use of 
 * lremove() where a pointer was being maintained (and used!) to the removed element's
 * stub. For such uses, we have the function lremove_without_freeing_stub() below.
 *
 * I checked the latest version of PostgreSQL, and the list code has changed significantly.
 * They also now free the stub on every lremove().
 *
 *                  - TCQ SR
 *  
 */
...
lremove(void *elem, List *list)
{
	...
	if (l != NIL)
	{
		...
		pfree(l); /* This missing pfree() was discovered on May 30, 2005 by Shariq Rizvi :) */
		/* If its absent, we leak two pointers worth of memory (8 bytes) per removal of a list element */
...
/*
 * lremove_without_freeing_stub
 * 
 * Same as above, but do not pfree() the stub. 
 *
*/
List *
lremove_without_freeing_stub(void *elem, List *list)
{
	List	   *l;
	List	   *prev = NIL;
	List	   *result = list;
...
/*
 * linsert
 *		Insert an element after List *place
 *              If place
 */

List *
linsert(void *obj, List *place, List *list)
{
  if (place == NIL) 
    {
      lappend(obj, list);
    ...
    {
      place->next = lcons(obj, place->next);
    }
  return list;
}
