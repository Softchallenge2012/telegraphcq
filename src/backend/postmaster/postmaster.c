 ...
 *
 * IDENTIFICATION
 *	  $Header: /project/eecs/db/cvsroot/postgres/src/backend/postmaster/postmaster.c,v 1.51 2005/05/04 02:28:38 phred Exp $
 *
 * NOTES
 *
 * Initialization:
...
#include "bootstrap/bootstrap.h"	// @dtcqSK 
...
/* User-level threads stuff. */
#include "telegraphcq/st_public.h"
...
typedef struct bkend
{
	...
	proc_t		proc_type;		/* type of process, so we know what it
	 * does */	/* @dtcqMS */
} Backend;
...
static int	BackendStartup(Port *port, proc_t ptype);	/* @dtcqSK */
...
PostmasterMain(int argc, char *argv[])
{
	...
	const char *numResultQueuesStr = NULL;
	...
	*original_extraoptions = '\0';
	...
	opterr = 1;
	/* Added new option t: for TelegraphCQ database */	/* @dtcqSK */
	/* Added new option r: for HiFi Glue */                 /* @dgludEW */
	/* Added new option Q: for setting numResultQueues */                 /* @dnumResultQueuesAE */
	while ((opt = getopt(argc, argv, "A:a:B:b:c:D:d:FgGHh:ik:lm:MN:no:p:Q:rSs-:t:u:Vv:w:xXyY:zZ")) != -1)
	{
		switch (opt)
		{
			case 'D':
	...
	while ((opt = getopt(argc, argv, "A:a:B:b:c:D:d:FgGHh:ik:lm:MN:no:p:Q:rSs-:t:u:Vv:w:xXyY:zZ")) != -1)
	{
		switch (opt)
		{
			...
			case 'F':
				...
				break;
			case 'g':
				elog(ERROR, "Eddy only supported in TelegraphCQ mode");
				enable_eddy = false;
				elog(NOTICE, "postmaster setting UseEddy");
				enable_eddy = true;
				break;
			case 'G':
				elog(NOTICE, "postmaster setting UseCQEddy");
				UseCQEddy = true;
				break;

				/* I need this for the hashing */
			case 'H':
				elog(NOTICE, "postmaster setting use-hash-group");
				UseHashGroup = true;
				break;
			...
			case 'p':
				...
				break;
			case 'Q':			/* @BdnumResultQueuesAE */
				elog(LOG, "Setting numResultQueues to %s ", optarg);
				SetConfigOption("numResultQueues", optarg, PGC_POSTMASTER, true);
				numResultQueues = atoi(GetConfigOption("numResultQueues"));
				break;
			case 'r':
				elog(NOTICE, "Hifi Glue only supported in telegraphCQ mode");
				enable_hifi = true;
				break;
			case 'w':
				SetConfigOption("telegraphcqwchport", optarg, PGC_POSTMASTER, PGC_S_ARGV);
				break;
			...
			case 't':			/* @BdtcqSK */
				elog(DEBUG1, "Starting TelegraphCQ on database %s ", optarg);
				SetConfigOption("telegraphcqdb", optarg, PGC_POSTMASTER, true);
				UseTelegraphCQ = true;
				break;
			case 'u':
				elog(DEBUG1, "Starting TelegraphCQ with userid %s ", optarg);
				SetConfigOption("telegraphcquser", optarg, PGC_POSTMASTER, true);
				break;
			case 'V':											/* @dsigmodSK */
				UseStaticPlan = true;
				elog(LOG, "UseStaticPlan = true;");          /* @dsigmodSK */
				break;
			case 'v':                        
				UseStaticPlanNumber = atoi(optarg);
				elog(LOG, "UseStaticPlanNumber = %d", UseStaticPlanNumber);
				elog(LOG, "Danger Will Robinson ! You better know whatchoo doin'");
				break;                                          /* @dsigmodSK */
			case 'x':
				UsePrecisionSharing = true;
				elog(LOG, "UsePrecisionSharing = true;");
				break;
			case 'X':
				UseTupleLineage = false;
				elog(LOG, "UseTupleLineage = false;");
				break;
			case 'y':
				enable_smart_routing = true;
				elog(LOG, "smart routing enabled");
				break;
			case 'Y':
				MaxTuplesScannedBeforeQuit = atoi(optarg);				
				elog(INFO,"MaxTuplesScannedBeforeQuit = %d", MaxTuplesScannedBeforeQuit);
				break;
			case 'z':
				UseOperatorPrecedence = true;
				elog(LOG, "UseOperatorPrecedence = true;");
				break;
			case 'Z':
				UseSuppressOutput = true;
				elog(LOG, "UseSuppressOutput = true;");
				break;
			default:
				fprintf(stderr, gettext("Try '%s --help' for more information.\n"), progname);
				ExitPostmaster(1);
	...
	Assert(UseTupleLineage || (! UsePrecisionSharing));
	 ...
	 * Set numResultQueues
	 */
	numResultQueuesStr = GetConfigOption("numResultQueues");

	if (numResultQueuesStr && strlen(numResultQueuesStr) > 0)
		numResultQueues = atoi(numResultQueuesStr);
	else 
	  elog(LOG, "numResultQueues option not valid!");
	// The default value (32) is set in globals.c
	elog(LOG, "postmaster: Number of result queues set to %d from %s", numResultQueues, numResultQueuesStr);
	...
	/*
	 * @BdtcqSK Time to start the TelegraphCQ Executor !!
	 */
	if (UseTelegraphCQ)
	{
		int			tcqstartupretrycount = 10;
		struct timeval now,
					later;
		struct timezone tz;

		/*
		 * We need the seed to be selected before backend startup is
		 * called, otherwise an an assertion is hit in PostmasterRandom().
		 * I did not want to modify PostmasterRandom to automatically
		 * select a seed. In general, it ought to be done outside.
		 */
		while (random_seed == 0)
		{
		...
		/*
		 * TelegraphCQ processes must wait for the Postgres startup
		 * process to finish before launching the wrapper clearinghouse
		...
		{
			/*
			 * elog(DEBUG1, "Waiting for startup -- Sleeping for 2 seconds
			 * ... ");
			 */
/*			 sleep(2); */
			tcq_usleep(1000);
		...
		{
			elog(NOTICE,
				 "Postgres was stated in TelegraphCQ mode, but the CQEddy "
				 "will not be used.  Did you forget the -G flag when starting "
				 "the postmaster ?");
		}
		...
		{
			/*
			 *  We initialize Judy so that it can use the PostgreSQL memory
			 *  manager
			 */
			JudySetMemoryHelpers(JudyPalloc,JudyPFree);
			
			/*
			 * Initialize the TopSharedMemoryContext, and the TelegraphCQ
			 * shared memory structures prior to forking any backend
			 * processes.
			 */
			...
			/* Create both a tcq backend and wrapper clearinghouse */
			BackendStartup(NULL, PROC_TCQ_BACKEND);
			BackendStartup(NULL, PROC_TCQ_WRAP_CH);
			if (enable_hifi)
			{
				elog(DEBUG1, "calling backendstartup with glue");
				BackendStartup(NULL, PROC_HIFI_GLUE);
			}
			elog(DEBUG1, "waiting for the TelegraphCQ backend and wrapperclearinghouse to startup");
			elog(DEBUG1, "For some microseconds ... ");
			tcqstartupretrycount = 20000;
			while (ShutdownPID == 0 &&
				   (*init_plumbing == false ||
			...
			{
				tcq_usleep(1000);
				tcqstartupretrycount--;
				if (tcqstartupretrycount == 0)
				{
					elog(NOTICE, "TelegraphCQ processes failed to start.");
			...
			if (tcqstartupretrycount > 0 &&
				ShutdownPID == 0)
				elog(NOTICE, "TelegraphCQ processes successfully started!");
...
#ifdef __linux__
/*																	 //@BdeddySK
 * Handle gprof fork ugliness
 */
extern void _start(void), etext(void);	/* @EdeddySK */
#endif
static void
pmdaemonize(int argc, char *argv[])
{
	...
	/*
	 * Check if the call to setitimer in 7.3.2 code is equivalent to
	 * monstartup
	 */
	monstartup((u_long) &_start, (u_long) &etext);		/* @deddySK */
#endif
...
usage(const char *progname)
{
	...
	printf(gettext("  -Q 1-256        maximum number of result queues. default = %d\n"), numResultQueues);
	printf(gettext("  -r              enable hifi glue\n"));
...
ServerLoop(void)
{
	...
	for (;;)
	{
			...
			&& FD_ISSET(ServerSock_UNIX, &rmask))
		{
			...
			if (port)
			{
				BackendStartup(port,	/* @dtcqMS */
							   (UseTelegraphCQ ? PROC_TCQ_FRONTEND :
								PROC_POSTGRES)
					);
...
reset_shared(unsigned short port)
{
	 ...
	 */
	elog(DEBUG1, "reset_shared: Calling CreateSharedMemoryAndSemaphores");
	CreateSharedMemoryAndSemaphores(false, MaxBackends, port);
	elog(DEBUG1, "reset_shared: Done calling CreateSharedMemoryAndSemaphores");
}
...
pmdie(SIGNAL_ARGS)
{
	...
	switch (postgres_signal_arg)
	{
		case SIGTERM:
			...
			elog(DEBUG1, "smart shutdown request");
			if (UseTelegraphCQ)
			{
				bool		ret = false;
				...
				cqe.command = SHUTDOWN;
				if (p_control_queue && *p_control_queue)
				{
					ret = enqueue(*p_control_queue,
								  (char *) &cqe, false,
								  &controlqentry_serializer);
					elog(INFO,
						 "postmaster: (pmdie) sent shutdown request to tcqbackend. ret=%d",
						 ret);
					wakeup_eddy();
				}

				if (p_wrap_control_queue && *p_wrap_control_queue)
				{

					ret = enqueue(*p_wrap_control_queue,
						(char *) &cqe, false, &controlqentry_serializer);
					elog(DEBUG1,
						 "sent shutdown request to the wrapper clearinghouse. ret=%d",
						 ret);
				}
...
reaper(SIGNAL_ARGS)
{
	...
	if (FatalError)
	{
		...
		if (DLGetHead(BackendList) || StartupPID > 0 || ShutdownPID > 0)
			goto reaper_done;
		elog(DEBUG1, "all server processes terminated; reinitializing shared memory and semaphores");
		if (UseTelegraphCQ)
		{
			elog(NOTICE,
				 "TelegraphCQ shutting down because a fatal error occured in some process");
			ExitPostmaster(1);
		}
		shmem_exit(0);
		reset_shared(PostPortNumber);
			...
			int exitstatus)		/* child's exit status. */
{
	...
	while (curr)
	{
		bp = (Backend *) DLE_VAL(curr);
		if (bp->pid == pid)
			elog(DEBUG1, "dead child process type: %d", bp->proc_type);
		curr = DLGetSucc(curr);
	}
 ...
 *
 * If telegraphcq is TRUE, then port is ignored
 *
 * returns: STATUS_ERROR if the fork/exec failed, STATUS_OK otherwise.
 *
 */
static int
BackendStartup(Port *port, proc_t ptype)		/* @dtcqMS */
{
	...
	/*
	 * Just in case we need to know the type of backend it is for later
	 * purposes
	 */
	bn->proc_type = (ptype < 0 || ptype > PROC_POSTGRES) ?		/* @dtcqMS */
		PROC_POSTGRES : ptype;
...
#ifdef LINUX_PROFILE
/* #ifdef __linux__ */
		elog(DEBUG1, "Doing a monstartup ... ");
		monstartup((u_long) &_start, (u_long) &etext);	/* @deddySK */
/* #endif */
		setitimer(ITIMER_PROF, &prof_itimer, NULL);
#endif
		...
		/* We need to se the process type before we do anything */
		MyProcessType = ptype;	/* @BdtcqMS */
		...
		switch (ptype)
		{
			case PROC_TCQ_WRAP_CH:
				{
                    elog(LOG, 
                        "TelegraphCQ wrapper clearinghouse process id: %d",
							(int) getpid());
					/*elog(DEBUG1, "Going to sleep for the debugger ... ");*/
/*					sleep(6000); */
					/*elog(DEBUG1, "Done sleeping !");*/
				}

				status = DoTelegraphCQ();
				break;

			case PROC_TCQ_BACKEND:
				{
					elog(LOG, "TelegraphCQ backend process id: %d",
							(int) getpid());
					/*elog(DEBUG1, "Going to sleep for the debugger ... ");*/
/*					sleep(6000); */
					/*elog(DEBUG1, "Done sleeping !");*/
			...
			case PROC_TCQ_FRONTEND:
				{
					...
					elog(LOG, "TelegraphCQ frontend process id: %d",
							(int) getpid());
					/*elog(DEBUG1, "Going to sleep for the debugger ... ");*/
/*					sleep(60); */
					/*elog(DEBUG1, "Done sleeping !");*/
			...
			default:
				{
					...
					sprintf(msg,
							"Postgres process id: %d\n",
							(int) getpid());
					elog(DEBUG1, msg);
					/* sleep(50); */
				}

				MyProcessType = PROC_POSTGRES;
				status = DoBackend(port);
				break;
	...
	if (server_min_messages < DEBUG1)
	{
		switch (ptype)
		{
			case PROC_TCQ_WRAP_CH:
				elog(DEBUG1,
					 "BackendStartup: started WrapperCH process: forked pid=%d",
					 pid);
				break;

			case PROC_TCQ_BACKEND:
				elog(DEBUG1,
					 "BackendStartup: started TelegraphCQ backend: forked pid=%d",
					 pid);
				break;

			case PROC_TCQ_FRONTEND:
				elog(DEBUG1,
					 "BackendStartup: started TelegraphCQ frontend: forked pid=%d socket=%d",
					 pid, port->sock);
				break;

			case PROC_POSTGRES:
			default:
				elog(DEBUG1, "BackendStartup: forked pid=%d socket=%d",
					 pid, port->sock);
				break;
...
DoBackend(Port *port)
{
	...
	return (PostgresMain(ac, av, port->user));	/* @dtcqSK */
}
...
DoTelegraphCQ()
{
	...
	/* Send output to the new command destination */
	whereToSendOutput = Remote; /* now safe to elog to client */
	...
	StrNCpy(dbbuf, GetConfigOption("telegraphcqdb"), ARGV_SIZE);
	av[ac++] = dbbuf;
	...
	switch (MyProcessType)
	{
		...
		case PROC_TCQ_BACKEND:
			return (TelegraphCQMain(ac, av));
			break;

		case PROC_TCQ_WRAP_CH:
			return (WrapCHMain(ac, av));
			break;

		case PROC_HIFI_GLUE:
			return (HiFiGlueMain(ac, av));
			break;
		default:
			elog(ERROR, "Unknown process type. Should not get here.");
			return 1;
			break;
	}
