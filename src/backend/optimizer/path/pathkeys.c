 ...
 *
 * IDENTIFICATION
 *	  $Header: /project/eecs/db/cvsroot/postgres/src/backend/optimizer/path/pathkeys.c,v 1.4 2005/11/25 20:09:23 rizvi Exp $
 *
 *-------------------------------------------------------------------------
 */
#include "postgres.h"
...
add_equijoined_keys(Query *root, RestrictInfo *restrictinfo)
{
	...
	foreach(cursetlink, root->equi_key_list)
	{
		...
		if (item1here || item2here)
		{
			 ...
			 * break.
			 */
			root->equi_key_list = lremove_without_freeing_stub(curset, root->equi_key_list); /* Using the correct version of lremove - TCQ SR */
