 ...
 *
 * IDENTIFICATION
 *	  $Header: /project/eecs/db/cvsroot/postgres/src/backend/optimizer/path/costsize.c,v 1.11 2004/03/24 08:10:57 chungwu Exp $
 *
 *-------------------------------------------------------------------------
 */
...
#define ALLOW_COMPLEX_KEYS /* FIXME defined in multiple files, be careful -- Amol */
...
bool		orig_enable_set = false;
bool		orig_enable_seqscan = true;
bool		orig_enable_indexscan = true;
bool		orig_enable_tidscan = true;
bool		orig_enable_sort = true;
bool		orig_enable_nestloop = true;
bool		orig_enable_mergejoin = true;
bool		orig_enable_hashjoin = true;
			   ...
			   List *innersortkeys)
{
	 ...
	 * cpu_operator_cost) for each tuple in the two source relations. This
...
#ifndef ALLOW_COMPLEX_KEYS
	/* these must be OK, since check_hashjoinable accepted the clause */
	left = get_leftop(restrictinfo->clause);
	right = get_rightop(restrictinfo->clause);
#else
    left = lfirst(pull_var_clause((Node*)get_leftop(restrictinfo->clause), false));
    right = lfirst(pull_var_clause((Node*)get_rightop(restrictinfo->clause), false));
#endif
