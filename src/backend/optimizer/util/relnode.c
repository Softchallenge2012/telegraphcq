 ...
 *
 * IDENTIFICATION
 *	  $Header: /project/eecs/db/cvsroot/postgres/src/backend/optimizer/util/relnode.c,v 1.3 2004/03/24 08:10:59 chungwu Exp $
 *
 *-------------------------------------------------------------------------
 */
#include "postgres.h"
						   ...
						   JoinType jointype)
{
	 ...
	 * equal.
	 *
	 * Weird special case: if we have two clauses that seem redundant except
	 * one is pushed down into an outer join and the other isn't, then
	 * they're not really redundant, because one constrains the joined
	 * rows after addition of null fill rows, and the other doesn't.
	 */
	foreach(item, rlist)
	{
