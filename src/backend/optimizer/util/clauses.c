 ...
 *
 * IDENTIFICATION
 *	  $Header: /project/eecs/db/cvsroot/postgres/src/backend/optimizer/util/clauses.c,v 1.5 2005/06/13 02:59:27 phred Exp $
 *
 * HISTORY
					   ...
					   void *context)
{
	...
	case T_EventExpr:
	  {
	    if(((EventExpr *)node)->event_op == EVENT_SIMPLE) {
	      if(walker((Node *)((EventExpr *)node)->var, context))
		return true;
	    }
	    else
	      {
		expression_tree_walker((Node *)((EventExpr *)node)->lchild, walker, context);
		expression_tree_walker((Node *)((EventExpr *)node)->rchild, walker, context);
		return true;
	      }
	  }
	  break;
