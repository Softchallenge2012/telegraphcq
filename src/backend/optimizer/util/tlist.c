 ...
 *
 * IDENTIFICATION
 *	  $Header: /project/eecs/db/cvsroot/postgres/src/backend/optimizer/util/tlist.c,v 1.7 2004/03/24 08:10:59 chungwu Exp $
 *
 *-------------------------------------------------------------------------
 */
#include "postgres.h"
...
/*
 * adjustVarWithTlist
 *		find a var node in a targetlist based on its old varno and varattno
 *		if found, adjust its varattno to the position of the targetlist entry
 *		and return true.  otherwise, return false.
 *
 */
bool
adjustVarWithTlist(Var *v, List *targetList)
{
	List	   *l;
	AttrNumber	attno = 1;
	...
	foreach(l, targetList)
	{
		...
		if (IsA(tle->expr, Var))
		{
			...
			if (v->varnoold == tleVar->varnoold &&
				v->varoattno == tleVar->varoattno)
			{
				v->varattno = attno;
				return true;
			}
