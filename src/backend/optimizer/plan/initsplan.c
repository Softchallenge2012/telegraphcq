 ...
 *
 * IDENTIFICATION
 *	  $Header: /project/eecs/db/cvsroot/postgres/src/backend/optimizer/plan/initsplan.c,v 1.5 2004/03/24 08:10:58 chungwu Exp $
 *
 *-------------------------------------------------------------------------
 */
#include "postgres.h"
...
#ifndef ALLOW_COMPLEX_KEYS /* Amol */
	if (!IsA(left, Var) ||!IsA(right, Var))
		return;
	opno = ((Oper *) clause->oper)->opno;
...
#else
    {
        List *left_var_list = pull_var_clause((Node*)left, false);
        List *right_var_list = pull_var_clause((Node*)right, false);
        Var *left_var, *right_var;
        if( (length(left_var_list) != 1) || (length(right_var_list) != 1))
            return;
        left_var = lfirst(left_var_list);
        right_var = lfirst(right_var_list);
