 ...
 *
 * IDENTIFICATION
 *	  $Header: /project/eecs/db/cvsroot/postgres/src/backend/optimizer/plan/setrefs.c,v 1.9 2004/03/24 08:10:59 chungwu Exp $
 *
 *-------------------------------------------------------------------------
 */
#include "postgres.h"
...
Node	   *eddy_change_var_references(Eddy * plan, Node *node);		/* @dstemsAD */
...
set_plan_references(Plan *plan, List *rtable)
{
	...
	switch (nodeTag(plan))
	{
		...
		case T_Eddy:
			...
			/*
			 * For an Eddy, there are a couple of things we must do. 1)
			 * fix_expr_references in all quals, 2) join_references to
			 * make sure that the quals are referring to the correct
			 * places. 3) recurse on SteMs.
			 *
			 * NOTE: It seems that the number 2 above is typically done while
			 * creating the plan. So we will also do this there.
			 */
			fix_expr_references(plan, (Node *) plan->targetlist);
			fix_expr_references(plan, (Node *) plan->qual);
			plan->targetlist =
				(List *) eddy_change_var_references((Eddy *) plan, (Node *) plan->targetlist);
			break;
		case T_SteM:

			/*
			 * SteMs don't have targetlists so no change_var_references
			 * needed.
			 */

			/*
			 * innerhashkey and outerhashkey have already been set
			 * properly. See note above.
			 */

			fix_expr_references(plan, (Node *) ((SteM *) plan)->joinquals);
			fix_expr_references(plan, (Node *) ((SteM *) plan)->stemclauses);
			fix_expr_references(plan, (Node *) ((SteM *) plan)->outerhashkey);
			fix_expr_references(plan, (Node *) ((SteM *) plan)->innerhashkey);
            break;

        case T_Filter :
			fix_expr_references(plan, (Node *) ((Filter *) plan)->filterqual);
            break;
	...
	/*
	 * For Eddy, we need to recurse on the modules and sources. @dstemsAD
	 */
	if (IsA(plan, Eddy))
	{
		Eddy	   *eddy = (Eddy *) plan;
		...
		for (i = 0; i < eddy->numSources; i++)
			set_plan_references(eddy->sources[i], rtable);
		for (i = 0; i < eddy->numModules; i++)
			set_plan_references(eddy->modules[i], rtable);
	}

	/*
	 * For SteM, we need to recurse on the innerNode. XXX: Why is
	 * innerNode not left- or right-tree ?
	 */
	if (IsA(plan, SteM))
		set_plan_references(((SteM *) plan)->innerNode, rtable);
