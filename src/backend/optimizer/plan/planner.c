 ...
 *
 * IDENTIFICATION
 *	  $Header: /project/eecs/db/cvsroot/postgres/src/backend/optimizer/plan/planner.c,v 1.10 2004/03/24 08:10:58 chungwu Exp $
 *
 *-------------------------------------------------------------------------
 */
...
planner(Query *parse)
{
	...
	/* printf("SUBPLANNER: PLAN BEFORE SETTING PLAN REFERENCES\n"); */
	/* pprint(result_plan); */

	/* final cleanup of the plan */
	set_plan_references(result_plan, parse->rtable);

	/* printf("SUBPLANNER: PLAN AFTER SETTING PLAN REFERENCES\n"); */
	/* pprint(result_plan); */
...
grouping_planner(Query *parse, double tuple_fraction)
{
	...
	else
	{
		 ...
		 */
		sub_tlist = make_subplanTargetList(parse, tlist, &groupColIdx, parse->windowClause);
					   ...
					   WindowClause * windowClause)
{
	...
	*groupColIdx = NULL;
	...
	{
		/* make sure timestamp columns are included in targetlist */
		List	   *wl;
		int			next_resdomno = length(sub_tlist) + 1;

		foreach(wl, windowClause->windowExprs)
		{
			WindowExpr *we = (WindowExpr *) lfirst(wl);

			if (!tlistentry_member((Node *) we->tsvar, sub_tlist))
			{
				Resdom	   *r;
				...
				sub_tlist = lappend(sub_tlist,
							  makeTargetEntry(r, copyObject(we->tsvar)));
			}
		}
	}
	/* @EwndaggWH */
