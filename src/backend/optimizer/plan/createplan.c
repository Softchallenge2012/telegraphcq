 ...
 *
 * IDENTIFICATION
 *	  $Header: /project/eecs/db/cvsroot/postgres/src/backend/optimizer/plan/createplan.c,v 1.13 2004/03/24 08:10:58 chungwu Exp $
 *
 *-------------------------------------------------------------------------
 */
#include "postgres.h"
...
#else
        op = lfirst(pull_var_clause((Node*)get_rightop(clause), false));
#endif
			 ...
			 Index scanrelid)
{
	...
	node->dontProject = false;	/* @deddySK */
			   ...
			   ScanDirection indexscandir)
{
	...
	node->scan.dontProject = false;		/* @deddySK */
