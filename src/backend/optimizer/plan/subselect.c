 ...
 *
 * IDENTIFICATION
 *	  $Header: /project/eecs/db/cvsroot/postgres/src/backend/optimizer/plan/subselect.c,v 1.8 2004/03/24 08:10:59 chungwu Exp $
 *
 *-------------------------------------------------------------------------
 */
#include "postgres.h"
...
SS_finalize_plan(Plan *plan, List *rtable)
{
	...
	switch (nodeTag(plan))
	{
		...
		case T_Eddy:
			...
			/*
			 * Nothing here really. plan.qual is already taken care of.
			 * @dstemsAD
			 */
			break;

		case T_SteM:
			/* stemclauses, innerhashkey, outerhashkey, joinquals. */
			finalize_primnode((Node *) ((SteM *) plan)->innerhashkey, &results);
			finalize_primnode((Node *) ((SteM *) plan)->outerhashkey, &results);
			finalize_primnode((Node *) ((SteM *) plan)->stemclauses, &results);
			finalize_primnode((Node *) ((SteM *) plan)->joinquals, &results);
	...
	/** Eddy and SteM have their own recursion semantics -- similar to set_plan_references. @dstemsAD */
	if (IsA(plan, Eddy))
	{
		Eddy	   *eddy = (Eddy *) plan;
		int			i;
		...
		for (i = 0; i < eddy->numSources; i++)
			results.paramids = set_unioni(results.paramids,
									   SS_finalize_plan(eddy->sources[i],
														rtable));
		for (i = 0; i < eddy->numModules; i++)
			results.paramids = set_unioni(results.paramids,
									   SS_finalize_plan(eddy->modules[i],
														rtable));
	}
	...
	{
		results.paramids = set_unioni(results.paramids,
							 SS_finalize_plan(((SteM *) plan)->innerNode,
											  rtable));
	}
