 ...
 *
 * IDENTIFICATION
 *	  $Header: /project/eecs/db/cvsroot/postgres/src/backend/optimizer/plan/planmain.c,v 1.8 2004/03/24 08:10:58 chungwu Exp $
 *
 *-------------------------------------------------------------------------
 */
#include "postgres.h"

#include "miscadmin.h"			/* needed for enable_eddy - @dstemsAD */
...
/** Eddy subplanner -- in createeddy.c @dstemsAD */
			  ...
			  double tuple_fraction)
{
	...
	/* printf("QUERY_PLANNER: ROOT BEFORE CALLING SUBPLANNER\n"); */
	/* pprint(root); */

	/*
	 * Choose the best access path and build a plan for it.
	 */
	if (enable_eddy && UseFastPathEddyCreate)
		subplan = eddy_subplanner(root, var_only_tlist);
	else
		subplan = subplanner(root, var_only_tlist, tuple_fraction);

	/* printf("QUERY_PLANNER: PLAN AFTER CALLING SUBPLANNER\n"); */
	/* pprint(subplan); */
		   ...
		   double tuple_fraction)
{
	...
	/* printf("SUBPLANNER: QUERY AFTER BASERELS HAVE BEEN BUILT\n"); */
	/* pprint(root); */
