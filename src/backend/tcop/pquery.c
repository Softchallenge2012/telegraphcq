 ...
 *
 * IDENTIFICATION
 *	  $Header: /project/eecs/db/cvsroot/postgres/src/backend/tcop/pquery.c,v 1.11 2004/05/15 18:57:29 chungwu Exp $
 *
 *-------------------------------------------------------------------------
 */
...
CreateExecutorState(void)
{
	...
	/* Initialize the outputBufferState to default values */
	state->outputBufferState = (OutputBufferState*)palloc(sizeof(OutputBufferState));
	MemSet((char*)state->outputBufferState, 0, sizeof(OutputBufferState));
	state->outputBufferState->outputBuffer = NULL;
	state->outputBufferState->outputScanner = NULL;
	state->outputBufferState->outputRelid = 0;
	state->outputBufferState->outputWindowStartTime = 0;	
	state->outputBufferState->timeMoveDirection = NoMovementScanDirection;
	state->outputBufferState->cursorType = CURSORKIND_UNKNOWN;
