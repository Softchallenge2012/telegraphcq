 ...
 *
 * IDENTIFICATION
 *	  $Header: /project/eecs/db/cvsroot/postgres/src/backend/tcop/fastpath.c,v 1.3 2004/03/24 08:11:10 chungwu Exp $
 *
 * NOTES
...
HandleFunctionRequest(void)
{
	...
	/*
	 * Set up a query snapshot in case function needs one.	(It is not
	 * safe to do this if we are in transaction-abort state, so we have to
	 * postpone it till now.  Ugh.)
	 */
	SetQuerySnapshot();
