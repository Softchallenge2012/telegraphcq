 ...
 *
 * IDENTIFICATION
 *	  $Header: /project/eecs/db/cvsroot/postgres/src/backend/tcop/utility.c,v 1.17 2006/02/06 01:19:46 phred Exp $
 *
 *-------------------------------------------------------------------------
 */
#include "postgres.h"
...
static struct kindstrings kindstringarray[] = {
	...
	{RELKIND_STREAM, "a", "stream", "STREAM"},
	{'\0', "a", "???", "???"}
};
...
static void
CheckDropStreamPermissions(RangeVar *rel)
{
	struct kindstrings *rentry;
	Oid			relOid;
	HeapTuple	tuple;
	...
	if (!IsAStream(classform->relkind))
		DropErrorMsg(rel->relname, classform->relkind, rentry->kind);
		...
		case T_FetchStmt:
			{
				...
				/* Either we perform a time move, for r/i/dstream
				 * cursor, or a normal fetch operation
				 */
				if (stmt->ismove && stmt->moveInterval != NULL)
				{
				  PerformPortalTimeMove(portalName,
							forward,
							stmt->moveInterval,
							completionTag);
				}
				...
				{
				  PerformPortalFetch(portalName, forward, stmt->iswindow,
                          count, (stmt->ismove) ? None : dest,
                          completionTag);
				}
			}
			break;
...
/* @BdwchMS */
		...
		case T_CreateStrmStmt:
			...
			CreateStream((CreateStrmStmt *) parsetree);
...
/* @EdwchMS */
		...
		case T_CreateWrapperStmt:
			CreateWrapper((CreateWrapperStmt *) parsetree);
			CommandCounterIncrement();
			break;
		...
		case T_DropStmt:
			{
				...
				foreach(arg, stmt->objects)
				{
					...
					switch (stmt->removeType)
					{
						...
						case DROP_STREAM:
							rel = makeRangeVarFromNameList(names);
							CheckDropStreamPermissions(rel);
							DropStream(rel, stmt->behavior);
							break;
						case DROP_WRAPPER:
							DropWrapper(names);
							break;
		...
		case T_AlterStreamStmt:
			{
				AlterStreamStmt *stmt = (AlterStreamStmt *) parsetree;

				switch (stmt->subtype)
				{

					case 'w':	/* ALTER TABLE DROP WRAPPER */
						AlterStreamWrapper(stmt->stream,
										   stmt->wrapper,
										   stmt->bindings,
								   stmt->info,
								   false);
						break;
					case 'W':	/* ALTER TABLE ADD WRAPPER */
						AlterStreamWrapper(stmt->stream,
										   stmt->wrapper,
										   stmt->bindings,
								   stmt->info,
						...
						break;
					default:
						elog(ERROR, "'unknown subtype '%c' in ALTER STREAM'", stmt->subtype);
				}
				break;
			}
