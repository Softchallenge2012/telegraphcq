 ...
 *
 * IDENTIFICATION
 *	  $Header: /project/eecs/db/cvsroot/postgres/src/backend/storage/buffer/freelist.c,v 1.4 2004/07/15 20:39:56 owenc Exp $
 *
 *-------------------------------------------------------------------------
 */
/*
...
PrintBuffers()
{
	...
	for (;;)
	{
		...
		printf("[%-2d] id=%d, blockNum=%d, flags=0x%x, refcnt=%d %ld, nxt=%ld prv=%ld)\n",
			   i, buf->buf_id, buf->tag.blockNum,
			   buf->flags, buf->refcount, PrivateRefCount[i],
			   (long int)buf->freeNext, (long int)buf->freePrev);
