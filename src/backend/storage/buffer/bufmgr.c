 ...
 *
 * IDENTIFICATION
 *	  $Header: /project/eecs/db/cvsroot/postgres/src/backend/storage/buffer/bufmgr.c,v 1.2 2004/03/24 08:11:06 chungwu Exp $
 *
 *-------------------------------------------------------------------------
 */
/*
...
/* 
 * undirty_buffers
 * provides functionality for UndirtyBuffers and release_and_read_buffers
 * hasLock is true if the process already did LWLockAcquire on BufMgrLock
 */
void
undirty_buffers(RelFileNode rnode, BlockNumber start, 
		BlockNumber end, bool hasLock)
{
  ...
  /* use the BufferTag as lookup key to find buffers to undirty */
  tag.rnode = rnode;
...
/* returns true if the given ProgressArray has no useful entry (no
 * scan module is using this relation
 */
bool ProgressArrayUnused(HeapProgressArray array)
{
  int i=0;
...
/* 
 * release_and_read_buffer -- provides common functionality for 
 * ReleaseAndReadBuffer and ReleaseReadAndUndirtyBuffer.  Does the same thing
 * as the previous ReleaseAndReadBuffer, but takes in an additional flag
 * to indicate whether the buffer should be undirtied if moving to a 
 * different buffer.
 */			    
Buffer
release_and_read_buffer(Buffer buffer,
			Relation relation,
			BlockNumber blockNum,
			bool attemptUndirty,
			int id)
{
	...
	MemoryContext old;
	int minBlock, numOfMin;
	HeapProgressLookupEnt* p_ent;
	bool undirtyBuffers;

	/* DEBUG: this is unnecessary but useful for information printing */
	Buffer newBuffer;
	...
	if (BufferIsValid(buffer))
	{
	  ...
	  else
	  {
	    ...
	    /* at this point, we will be crossing from one block
	     * to the next.  Find out which blocks we need to undirty, 
	     * if any
	     */

	    if (attemptUndirty)
	    {
	      undirtyBuffers = false;
	      old = MemoryContextSwitchTo(TopSharedMemoryContext);
	      SpinLockAcquire(heapScanProgressTableSpinLock);

	      p_ent = ProgressArrayLookup(relation);
	      ...
	      ProgressArraySetValue(p_ent->progress, id, blockNum);

	      if (minBlock == bufHdr->tag.blockNum && numOfMin == 1)
	      {
		/* We are the single slowest scan module! */
		undirtyBuffers = true;
	      }
	    ...
	    if (PrivateRefCount[buffer - 1] > 1)
	    {
	      ...
	      if (attemptUndirty && undirtyBuffers)
	      {
		/* the executor still has a pin on the buffer, but we can
		 * safely clear the dirty bit anyway, since the executor will not
		 * be dirtying the buffer.
		 */
		undirty_buffers(bufHdr->tag.rnode, bufHdr->tag.blockNum, blockNum, false);
	      }
	    }
	    else
	    {
	      ...
	      /* we're crossing to another buffer, so undirty
	       * the previous one if so desired
	       */
	      if (attemptUndirty && undirtyBuffers)
	      {
		undirty_buffers(bufHdr->tag.rnode, bufHdr->tag.blockNum, blockNum, true);
	      }

	      UnpinBuffer(bufHdr);
	      newBuffer = ReadBufferInternal(relation, blockNum, true);
	...
	newBuffer = ReadBufferInternal(relation, blockNum, false);
