 ...
 *
 * IDENTIFICATION
 *	  $Header: /project/eecs/db/cvsroot/postgres/src/backend/commands/tablecmds.c,v 1.6 2004/05/15 18:57:23 chungwu Exp $
 *
 *-------------------------------------------------------------------------
 */
#include "postgres.h"
...
DefineRelation(CreateStmt *stmt, char relkind)
{
	...
	if (IsAStream(relkind))		/* @BdtcqSK */
	{
		/*
		 * @dfluxMS sorry, I don't need a timestamp column.
		 * if (!(descriptor->constr && tc_has_timestamp(descriptor->constr)))
		 * {
		 * elog(ERROR,
		 * "A STREAM must have one TIMESTAMPCOLUMN constraint column");
		 * }
		 */
	}							/* @EdtcqSK */
...
RemoveRelation(const RangeVar *relation, DropBehavior behavior)
{
	...
	/* Get the Oid of the relation, and use RemoveRelationWithOid */
	relOid = RangeVarGetRelid(relation, false);
	RemoveRelationWithOid(relOid, behavior);
}

/*
 * RemoveRelationWithOid
 * Deletes a relation with the given Oid
 */
void RemoveRelationWithOid(Oid relOid, DropBehavior behavior)
{
...
TruncateRelation(const RangeVar *relation)
{
	...
	/*
	 * Truncate within a transaction block is dangerous, because if the
	 * transaction is later rolled back we have no way to undo truncation
	 * of the relation's physical file.  Disallow it except for a rel
	 * created in the current xact (which would be deleted on abort,
	 * anyway).
	 */
	if (!rel->rd_isnew)
...
AlterTableAddCheckConstraint(Relation rel, Constraint *constr)
{
	...
	/*
	 * We need to make a parse state and range table to allow us to
	 * transformExpr and fix_opids to get a version of the expression we
	 * can pass to ExecQual
	 */
						...
						int numattrs, int16 *attnums)
{
	...
	foreach(indexoidscan, indexoidlist)
	{
			...
			VARSIZE(&indexStruct->indpred) <= VARHDRSZ)
		{
			...
			{
				/*
				 * The given attnum list may match the index columns in
				 * any order.  Check that each list is a subset of the
				 * other.
				 */
