 ...
 *
 * IDENTIFICATION
 *	  $Header: /project/eecs/db/cvsroot/postgres/src/backend/commands/copy.c,v 1.4 2004/07/15 20:39:53 owenc Exp $
 *
 *-------------------------------------------------------------------------
 */
#include "postgres.h"
...
DoCopy(const CopyStmt *stmt)
{
	...
	{							/* copy from file to database */
		if (rel->rd_rel->relkind != RELKIND_RELATION &&
			rel->rd_rel->relkind != RELKIND_ARCHIVEDSTREAM)
		{
		 ...
		 FILE *fp, char *delim, char *null_print)
{
	...
	for (i = 0; i < num_phys_attrs; i++)
	{
		...
		if (get_typtype(attr[i]->atttypid) == 'd')
		{
			 ...
			 * call and/or ConstraintTest nodes.)  The bottom of the
			 * expression is a Const node that we fill in with the actual
			 * datum during the data input loop.
			 *
			 * XXX to prevent premature constant folding in parse_coerce,
			 * pass in a NULL constant to start with.  See the comments in
			 * coerce_type; this should be changed someday to use some
			 * sort of Param node instead of a Const.
			 */
