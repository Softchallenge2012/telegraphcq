 ...
 *
 * IDENTIFICATION
 *	  $Header: /project/eecs/db/cvsroot/postgres/src/backend/commands/vacuum.c,v 1.3 2004/03/24 08:10:53 chungwu Exp $
 *
 *-------------------------------------------------------------------------
 */
#include "postgres.h"
	...
	foreach(cur, vrl)
	{
		...
		if (vacstmt->analyze)
		{
			...
			if (vacstmt->vacuum)
			{
				...
				SetQuerySnapshot();		/* might be needed for functional
										 * index */
			}
			else
