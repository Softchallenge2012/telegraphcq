 ...
 *
 * IDENTIFICATION
 *	  $Header: /project/eecs/db/cvsroot/postgres/src/backend/parser/parse_relation.c,v 1.11 2005/06/13 02:59:27 phred Exp $
 *
 *-------------------------------------------------------------------------
 */
#include "postgres.h"
 ...
 *	  Search for an unqualified column name.
 *	  If found, return the appropriate Var node (or expression).
 *	  If not found, return NULL.  If the name proves ambiguous, DO NOT raise an error 
 *             like colnametoVar, but return a list of Var's
 *
 *      Required for "partition by" clause, where we may have the same column name
 *      across several streams
 *
 *                                    - TCQ SR
 */
List *
colnameToVarList(ParseState *pstate, char *colname)
{
	List	   *result = NIL;
	ParseState *orig_pstate = pstate;
	...
	while (pstate != NULL && !result)
	{
		...
		foreach(ns, pstate->p_namespace)
		{
			...
			if (IsA(nsnode, RangeTblRef))
			{
				...
				elog(ERROR, "colnameToVarList: unexpected node type %d",
					 nodeTag(nsnode));

			if (newresult)
			{
			  result = lappend(result, newresult);
			}
		}

		if (!result)
		  elog(ERROR, "colnameToVarList: Column name %s not found in any range table entry", colname);
...
/*		rt_getrtr													 //@BdwndwSK
 *
 *		   Get the RangeTblRef for a particular Rangetable entry
 */
RangeTblRef *
rt_getrtr(List *rtbl, RangeTblEntry *rte)
{
	RangeTblRef *rtr;
	int			rtindex;

	rtindex = ptrMemberWhich(rte, rtbl);
