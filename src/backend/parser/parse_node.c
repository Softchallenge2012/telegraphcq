 ...
 *
 * IDENTIFICATION
 *	  $Header: /project/eecs/db/cvsroot/postgres/src/backend/parser/parse_node.c,v 1.12 2005/02/09 00:16:46 phred Exp $
 *
 *-------------------------------------------------------------------------
 */
#include "postgres.h"
...
#include "catalog/namespace.h"	/* @dtcqSK */
...
static void setOperatorType(List *names, Oper *newop);	/* @dcacqSK */
...
make_parsestate(ParseState *parentParseState)
{
	...
	pstate->windowClause=makeNode(WindowClause);

    /* The following statement leaks memory, but we'll assume that the 
     * parse context will go away soon anyway. */
    pstate->p_time = 
        DatumGetTimestamp(
            DirectFunctionCall1(text_timestamp,
                    DirectFunctionCall1(timeofday,PointerGetDatum(NULL))));
    /*
    elog(LOG, "Set parse time to %s",
            DatumGetCString(
            DirectFunctionCall1(timestamp_out, 
                TimestampGetDatum(pstate->p_time))));
                */
 ...
 * setOperatorType()
 *
 * This function stares at the opname and looks for one of:
 *
 *				  <,>,=,<=,>=,<>
 *
 * The optrtype field of the Oper node is set appropriately
 */
void
setOperatorType(List *names, Oper *newop)
{
	OptrType	ot = OTHER_OPTR;
	char	   *schemaname;
	char	   *opname;

	/* deconstruct the name list */
	DeconstructQualifiedName(names, &schemaname, &opname);

	/*
	 * Since we are only looking for operators we can safely (gulp) omit
	 * schemaname from consideration ... famous last words.
	 */
...
make_op(List *opname, Node *ltree, Node *rtree)
{
	...
	setOperatorType(opname, newop);		/* @dcacqSK */
