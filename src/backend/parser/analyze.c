 ...
 *
 *	$Header: /project/eecs/db/cvsroot/postgres/src/backend/parser/analyze.c,v 1.39 2006/02/06 01:19:46 phred Exp $
 *
 *-------------------------------------------------------------------------
 */
...
/* TelegraphCQ functions */
static bool isContinuousQuery(const Query *qry);
			  ...
			  List **extras_before, List **extras_after)
{
	...
	switch (nodeTag(parseTree))
	{
			 ...
			 */
		case T_CreateStmt:
		case T_CreateStrmStmt:	/* @dtstampSK */
	...
	if (result->commandType != CMD_SELECT)
		result->isContinuousQuery = false;
	return result;
}
...
makeObjectName(char *name1, char *name2, char *typename)
{
	...
	if (name1)
		name1chars = pg_mbcliplen((unsigned char*)name1, name1chars, name1chars);
	if (name2)
		name2chars = pg_mbcliplen((unsigned char*)name2, name2chars, name2chars);
						  ...
						  ColumnDef *column)
{
	...
	foreach(clist, column->constraints)
	{
		...
		switch (constraint->contype)
		{
			...
			case CONSTR_TIMESTAMPCOLUMN:		/* @BdtstampSK */
				if (saw_timestamp && column->is_timestamp)
				{
					elog(ERROR, "%s/ TIMESTAMP conflicting declaration for '%s.%s'",
						 cxt->stmtType, cxt->relation->relname, column->colname);
				}
				if (strcmp(column->colname, TCQTSATTRNAME) != 0)
				{
					elog(ERROR, "%s/  TIMESTAMPCOLUMN constraint for '%s.%s' "
						 " MUST  have  name '%s'",
						 cxt->stmtType, cxt->relation->relname,
						 column->colname, TCQTSATTRNAME);
				}
				column->is_timestamp = TRUE;
				saw_timestamp = true;
				break;			/* @EdtstampSK */
...
transformFKConstraints(ParseState *pstate, CreateStmtContext *cxt)
{
	 ...
	 * gin up an ALTER TABLE ADD CONSTRAINT command to execute after the
	 * basic CREATE TABLE is complete.
	 *
...
transformSelectStmt(ParseState *pstate, SelectStmt *stmt)
{
	...
	WindowExpr *we = NULL;
	Node *max = NULL;
	char *maxopname = "timestamp_larger";
	List *maxargs = NIL;

	qry->commandType = CMD_SELECT;

	/*
	 * Moved to bottom of this function, since we need the complete range
	 * table to determine whether a query is continuous.
	 * qry->isContinuousQuery=stmt->isContinuousQuery;
	 */
	if (stmt->portalname)
	{
		/* DECLARE CURSOR */
		...
		/* Set the cursorType to the specified type */
		qry->cursorType = stmt->cursorType;
	}
	else
	{
		/* SELECT */
		...
		/* Initialize cursorType to unspecified */
		qry->cursorType = CURSORKIND_UNKNOWN;
	}
	...
	qry->targetList = transformTargetList(pstate, stmt->targetList);
	qry->recurseRelation = transformRecurseRelation(pstate, stmt->resultClause, qry->targetList);
	...
	/* process the EVENT clause */
	qry->eventClause = transformEventClause(pstate, stmt->eventClause);  /* - TCQ SR */
	...
	qry->windowClause = pstate->windowClause;

	foreach(iter,qry->windowClause->windowExprs)
	  {
	    we = lfirst(iter);
	    if (max == NULL)
	      {
		max = (Node *) we->tsvar;
	      ...
	      {
		maxargs = makeList2((Node *) we->tsvar, max);
		max = ParseFuncOrColumn(pstate, makeList1(makeString(maxopname)),
					maxargs, false, false, true);
	      }
	  }
	qry->windowClause->max = max;

	qry->aggWinClause = stmt->aggWinClause;

	qry->total_machines = stmt->total_machines;

	if (stmt->forUpdate != NIL)
		transformForUpdate(qry, stmt->forUpdate);

	/*
	 * Check the data sources for this query.  If they are all tables,
	 * then the query is a traditional SELECT query.  If they are all
	 * streams, then the query is a continuous (e.g. what was formerly
	 * known as SELECTCQ) query. Mixtures of tables and streams are
	 * currently not supported.
	 */
	qry->isContinuousQuery = isContinuousQuery(qry);

	if (UseTelegraphCQ &&
        qry->isContinuousQuery &&
		(qry->windowClause == NULL) &&
		(qry->hasAggs ||
		 qry->distinctClause ||
		 qry->sortClause ||
		 qry->groupClause) && !(UseHashGroup))
		elog(ERROR, "Continuous queries without windows may not contain DISTINCT, GROUP BY, or ORDER BY clauses");
...
/* FUNCTION isContinuousQuery
 * ARGUMENTS: <qry> points to a mostly-analyzed Query object.
 * PRECONDITIONS: qry has a complete range table.
 * POSTCONDITIONS: Determines whether the query is a continuous (e.g. stream
 * semantics) query or not.
 */
static bool
isContinuousQuery(const Query *qry)
{
	List	   *rtable = qry->rtable;
	List	   *itr = NULL;
	RangeTblEntry *rtentry = NULL;
	Relation	reldesc;
	int			numStreamsFound = 0;
	int			numTablesFound = 0;

	/*
	 * Scan through the range table, counting how many streams and tables
	 * are found.
	 */
	foreach(itr, rtable)
	{
		...
		/* 
		if (rtenry->rtekind == RTE_SUBQUERY)
		{
			
		}
		*/
		
		reldesc = RelationIdGetRelation(rtentry->relid);

		if (reldesc == NULL)	/* XXX Found a subquery? */
			return false;
		...
		/* @tcqoc: make sure the refcount for this rel is OK. */
		if (reldesc)
			RelationClose(reldesc);
	}


	if (0 == numStreamsFound)	/* No streams */
		return false;
	else if (0 != numStreamsFound && 0 == numTablesFound)		/* Only streams */
		return true;
	else
/* streams and tables */
	{
		/*
		 * The query references both streams and tables, which is
		 * something that TelegraphCQ doesn't currently support.
		 */
		return true;
	}

	/* Unreachable, but we put a return statement here to satisfy gdb. */
	return false;
}
