 ...
 *
 * IDENTIFICATION
 *	  $Header: /project/eecs/db/cvsroot/postgres/src/backend/parser/parse_coerce.c,v 1.3 2004/03/24 08:11:00 chungwu Exp $
 *
 *-------------------------------------------------------------------------
 */
#include "postgres.h"
				   ...
				   CoercionForm cformat)
{
	...
	if (OidIsValid(funcId))
	{
		...
		/*
		 * If the input is a constant, apply the length coercion function
		 * now instead of delaying to runtime.
		 *
