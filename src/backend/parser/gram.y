...
%type <node>	stmt schema_stmt
		AlterDatabaseSetStmt AlterGroupStmt AlterStreamStmt
		AlterTableStmt AlterUserStmt AlterUserSetStmt
		AnalyzeStmt ClosePortalStmt ClusterStmt CommentStmt
		ConstraintsSetStmt CopyStmt CreateAsStmt CreateCastStmt
		CreateDomainStmt CreateGroupStmt CreateOpClassStmt CreatePLangStmt
		CreateSchemaStmt CreateSeqStmt CreateStmt CreateStrmStmt 
		CreateAssertStmt CreateTrigStmt CreateUserStmt CreateWrapperStmt
...
%type <node>   simple_select with_def_list with_def_item with_select
...
%type <str>		relation_name copy_file_name
				...
				index_name name function_name file_name wrapper_name 
...
%type <str>		opt_id	all_Op MathOp opt_name optwrapperinfo optaswrapperinfo SpecialRuleRelation
...
%type <list>	stmtblock stmtmulti
                ...
                wrapper_definition wrapper_definition_list  
				...
				execute_param_clause partition_clause col_list
...
%type <ival>	fetch_how_many declust_clause
...
%type <node>	columnDef
%type <defelt>	def_elem wrapper_def
...
%type <node>    event_clause event_expr
...
%type <node>	table_ref result_clause 
%type <jexpr>	joined_table
%type <range>	relation_expr
%type <target>	target_el insert_target_el update_target_el
%type <window>  window_expr
%type <window>  window_clause
%type <str>     optrangeby optslideby optstartat
%type <list>    optslices
%type <list>    slices_list
%type <value>   optslidetimeout
%type <agg_window> having_window_clause
%type <chr>     stream_type
%type <cursortype> cursor_stream_type
...
%type <list> optbindings binding_list
%type <node> binding

%type <overload_params> OptWithOverload OverloadParams
...
%token <keyword> ABORT_TRANS ABSOLUTE ACCESS ACTION ADD AFTER
	...
	COMMITTED CONSTRAINT CONSTRAINTS CONVERSION_P CONVERT COPY CQEDDY CREATE CREATEDB
...
/*                                                                     @BdwndwSK
 * Keywords for TelegraphCQ support (not in SQL92 reserved words)
 */

%token   SLIDE SLIDETIMEOUT RANGE WINDOW  ARCHIVED UNARCHIVED UNARCHIVED_DROP UNARCHIVED_SPILL TIMESTAMPCOLUMN STREAM WRAPPER WRAPPERINFO DECLUST BINDINGS RECURSIVE_RESULT RECURSIVE_NEXT ISTREAM RSTREAM DSTREAM OPTIONAL OVERLOAD KEEP DROP COUNTS MYHIST BLOCK REGHIST WAVELET SAMPLE EVENT PARTITION ANYONE FOLLOWS NOTFOLLOWS PRECEDES NOTPRECEDES SLICES
...
%left		UNION EXCEPT RECURSIVE_NEXT
%left		INTERSECT
%left		OR
%left		AND
%right		NOT
...
/*****************************************************************************
 *
 *	ALTER STREAM variations
 *
 *****************************************************************************/

AlterStreamStmt:
			/* ALTER STREAM <stream> ADD WRAPPER <wrappername> */
			ALTER STREAM relation_expr ADD WRAPPER wrapper_name optbindings optaswrapperinfo
                {
				    AlterStreamStmt *n = makeNode(AlterStreamStmt);
					n->subtype = 'W';
					if($3->catalogname && $3->schemaname)
					  n->stream = makeList3(makeString($3->catalogname),
											makeString($3->schemaname),
											makeString($3->relname));
					else if($3->schemaname)
					  {
						n->stream = makeList2(makeString($3->schemaname),
											  makeString($3->relname));
					  }
					else
					  {
						n->stream = makeList1(makeString($3->relname));
					  }
					
					n->wrapper = makeList1(makeString($6));
					n->bindings=$7;
					n->info=$8;
					$$ = (Node *)n;
				}
            /* ALTER STREAM <stream> DROP WRAPPER <wrappername> */
            | ALTER STREAM relation_expr DROP WRAPPER wrapper_name
			    {
				    AlterStreamStmt *n = makeNode(AlterStreamStmt);
					n->subtype = 'w';
										if($3->catalogname && $3->schemaname)
...
optbindings: WITH BINDINGS '(' binding_list ')'
{
  $$ = $4;
}
| /*empty*/
...
binding: columnElem 
	...
	| columnElem OPTIONAL DEFAULT Sconst
	{
		Binding *b = makeNode(Binding);
		b->column=$1;
		b->isOptional=true;
		b->def=(Value*)makeString($4);
		$$=(Node*)b;
	}
	;
...
CreateStrmStmt:  CREATE STREAM qualified_name '(' OptTableElementList ')' TYPE_P stream_type OptWithOverload
                     {
						 ...
						 ns->streamtype = $8;
						 ns->overload_params = $9;
						 $$ = (Node *) n;
					 }
             ;

OptWithOverload:
			ON OVERLOAD KEEP OverloadParams { $$ = $4; }
			| ON OVERLOAD DROP      { $$ = makeOverloadParams(OVERLOAD_DROP, NULL); }
			| ON OVERLOAD BLOCK     { $$ = makeOverloadParams(OVERLOAD_BLOCK, NULL); }
			| /*EMPTY*/             { $$ = makeOverloadParams(OVERLOAD_BLOCK, NULL); }
		;

OverloadParams:
			COUNTS    { $$ = makeOverloadParams(OVERLOAD_COUNTS, NULL); }
			| REGHIST { $$ = makeOverloadParams(OVERLOAD_REGHIST, NULL); }
			| MYHIST   { $$ = makeOverloadParams(OVERLOAD_MHIST, NULL); }
			| WAVELET '(' Sconst ')'
				{
					$$ = makeOverloadParams(OVERLOAD_WAVELET, $3);
				}
			| SAMPLE   { $$ = makeOverloadParams(OVERLOAD_SAMPLE, NULL); }
                ;

/* @EdwchMS */

stream_type:  ARCHIVED
{
	$$ = RELKIND_ARCHIVEDSTREAM;;
}
| UNARCHIVED
{
#ifdef BP_SPILL
	$$= RELKIND_UNARCHIVEDSTREAM_SPILL;
#else
	$$= RELKIND_UNARCHIVEDSTREAM_DROP;
#endif
...
optwrapperinfo: WITH WRAPPERINFO Sconst
{
  $$ = $3;
}
| /* empty */
...
optaswrapperinfo: WRAPPERINFO Sconst
{
  $$ = $2;
}
| /* empty */
...
CreateWrapperStmt: CREATE WRAPPER wrapper_name  wrapper_definition optwrapperinfo
{
...
wrapper_definition: '(' wrapper_definition_list ')' {$$=$2;}
;

wrapper_definition_list:
wrapper_def { $$ = makeList1($1); }
| wrapper_definition_list ',' wrapper_def
{
  $$ = lappend($1,$3);
}
;

wrapper_def: ColLabel '=' dotted_name
{
  $$ = makeDefElem($1, (Node*)$3);
}
   | ColLabel '=' type_name
{
  $$ = makeDefElem($1, (Node*)makeList1(makeString($3)));
}
;
...
FetchStmt:	FETCH direction fetch_how_many from_in name
			...
			| MOVE direction name TIME BY '[' Sconst ']'
                                {
				  ...
				  n->moveInterval = make_const(makeString($7));
				  n->moveInterval =
...
CursorStmt: DECLARE name opt_cursor CURSOR FOR SelectStmt
          ...
          | DECLARE name opt_cursor AS cursor_stream_type CURSOR FOR SelectStmt
                                {				 
...
cursor_stream_type: ISTREAM { $$ = CURSORKIND_ISTREAM; }
                  | RSTREAM { $$ = CURSORKIND_RSTREAM; }
                  | DSTREAM { $$ = CURSORKIND_DSTREAM; }
                  ;
...
simple_select:
			...
			into_clause from_clause partition_clause event_clause where_clause
			group_clause having_clause 
			having_window_clause declust_clause result_clause
				{
...
/* 					  elog(NOTICE, "The query has an event clause"); */
					...
					if(n->eventClause)
					  ((EventExpr *)n->eventClause)->partition = (List *)$6;
					else {
					  if($6)
			...
			| select_clause EXCEPT opt_all select_clause
				{
					$$ = makeSetOp(SETOP_EXCEPT, $3, $1, $4);
                       ...
                       | select_clause RECURSIVE_NEXT select_clause
			{
			  $$ = makeSetOp(SETOP_RECURSIVE_NEXT, TRUE, $1, $3);
			}
			;



with_select: WITH with_def_list select_with_parens
{
  $$ = makeSetOp(SETOP_RECURSIVE_NEXT, TRUE, $2, $3);
}
;
with_def_list: with_def_list with_def_item  
{
  $$= makeSetOp(SETOP_RECURSIVE_NEXT, TRUE, $2, $1);
}
| with_def_item
{
  $$ = $1;
...
with_def_item: qualified_name AS select_with_parens
{
	((SelectStmt*)$3)->resultClause=(Node*)$1;
	$$=$3;
}
  ;
...
partition_clause:
			PARTITION BY col_list	{ $$ = $3; }
			| /*EMPTY*/								{ $$ = NIL; }
		;


/*
 * Hack to determine number of machines.
 */

declust_clause: DECLUST '[' Iconst ',' Sconst ',' Sconst ']'
        {
			elog(DEBUG1, "DECLUST: %d, PLAN: %s", $3, $5);
			total_machines = $3;
			elog(DEBUG1, "Looking up ...");
			fdataflow = bsearch($5, fdflow_strs, FP_TOTAL_PLANS,
        ...
        | DECLUST '[' Iconst ']'
          {
			  elog(DEBUG1, "DECLUST: %d, PLAN: none", $3);
			  total_machines = $3;
			  is_pair = FALSE;
			  $$ = $3;
		  }
...
/* Another hack for specifying aggregation windows.  Replaced HAVING
   with USING to remove shift-reduce conflict. */

having_window_clause: USING WINDOW '[' Iconst ',' Iconst ']'
                      { 
						  AggWindowSpec *spec;
/* 						  elog(ERROR, "At the moment, this feature is disabled."); */
						  spec = palloc(sizeof(AggWindowSpec));
						  elog(DEBUG1, "AGG WINDOW: %d HOP: %d", $4, $6);
						  spec->window_interval = $4;
						  spec->hop_size = $6;
						  $$ = spec;
					  }
                      | /*EMPTY*/								{ $$ = NULL; }
...
/*                                                                   //@BdwndwSK
 * Support for TelegraphCQ WINDOW expressions
 *
 */ 

window_expr: '[' window_clause optslidetimeout optstartat ']' 
		     {
			   WindowExpr *we = $2;
...
window_clause:  optrangeby optslideby
                { 
 				  WindowExpr *we = makeNode(WindowExpr);
				  we->relation = NULL;
				  we->slices = NIL;
				  we->rangeby = makeString($1);
				  if ($2 != NULL) {
					we->slideby = makeString($2);
				  }
				  $$ = we;
				}
             |  optslices
...
optslices : SLICES slices_list { $$ = $2;  }
          ;

slices_list : Sconst 			     { $$ = makeList1(makeString($1)); }
            | slices_list ',' Sconst { $$ = lappend($1, makeString($3)); }
            ;

optrangeby: RANGE BY Sconst 
{
  $$ = $3;
}   
| /* empty */
...
optslidetimeout: SLIDETIMEOUT Iconst
{
  $$ = makeInteger($2);
} 
| /* empty */
...
/*
 * End support for TelegraphCQ WINDOW expressions                     @EdwndwSK 
 */ 
for_update_clause:
...
result_clause: RECURSIVE_RESULT IS table_ref
{
   $$ = $3;
}
|  /* empty */
 ...
 */
table_ref:	relation_expr window_expr
				{
				  $1->window = (Node*)$2;
				  if($2 != NULL)
				    {
			...
			| relation_expr alias_clause window_expr
				{
					$1->alias = $2;
					$1->window=(Node*)$3;
					if($3 != NULL)
			 ...
			 qualified_name
                               {
                                       ...
                                       n->stream_name = (RangeVar *)$1;
                                       n->event_op = EVENT_SIMPLE;
                                       $$ = (Node *)n;
                               }
                       | ANYONE '(' event_expr ',' event_expr ')'
                               {
                                       EventExpr *n = makeNode(EventExpr);
				       n->event_op = EVENT_ANYONE;
                                       n->lchild = (EventExpr *)$3;
                                       n->rchild = (EventExpr *)$5;
				       n->time_delay_val = NULL;
                                       $$ = (Node *)n;
                               }
                       | BOTH '(' event_expr ',' event_expr ',' SCONST ')'
                               {
                                       EventExpr *n = makeNode(EventExpr);
				       n->event_op = EVENT_BOTH;
                                       n->lchild = (EventExpr *)$3;
                                       n->rchild = (EventExpr *)$5;
				       n->time_delay_val = makeString($7);
                                       $$ = (Node *)n;
                               }
                       | FOLLOWS '(' event_expr ',' event_expr ',' SCONST  ')'
                               {
                                       EventExpr *n = makeNode(EventExpr);
                       ...
                       | NOTFOLLOWS '(' event_expr ',' event_expr ',' SCONST ')'
                               {
                                       EventExpr *n = makeNode(EventExpr);
                       ...
                       | PRECEDES '(' event_expr ',' event_expr ',' SCONST  ')'
                               {
                                       EventExpr *n = makeNode(EventExpr);
                       ...
                       | NOTPRECEDES '(' event_expr ',' event_expr ',' SCONST  ')'
                               {
                                       EventExpr *n = makeNode(EventExpr);
...
col_list:       IDENT {$$ = makeList1(makeString($1));}
                        | IDENT ',' col_list 
                                {
                                   $$ = lcons(makeString($1), $3);
				}
