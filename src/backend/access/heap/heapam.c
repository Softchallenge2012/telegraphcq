 ...
 *
 * IDENTIFICATION
 *	  $Header: /project/eecs/db/cvsroot/postgres/src/backend/access/heap/heapam.c,v 1.15 2004/03/24 08:08:05 chungwu Exp $
 *
 *
 * INTERFACE ROUTINES
...
{
	if (IsHeapAccessedStream(scan->rs_rd->rd_rel->relkind)) /* @BdtcqSK */
	{
		   ...
		   ScanKey key)
{
	...
	if (IsHeapAccessedStream(relation->rd_rel->relkind))	/* @BdtcqSK */
	{
		streamheapgettup(relation,
						 dir,
						 tuple,
						 buffer,
						 snapshot,
			   ...
			   int nkeys, ScanKey key)
{
	...
	if (IsHeapAccessedStream(relation->rd_rel->relkind))	/* @BdtcqSK */
	{
		return streamheap_beginscan(relation, snapshot, nkeys, key);
	}							/* @EdtcqSK */
			...
			ScanKey key)
{
	if (IsHeapAccessedStream(scan->rs_rd->rd_rel->relkind)) /* @BdtcqSK */
	{
		return streamheap_rescan(scan, key);
	}							/* @EdtcqSK */
...
{
	if (IsHeapAccessedStream(scan->rs_rd->rd_rel->relkind))
	{
		streamheap_endscan(scan);
		return;
	}
...
heap_getnext(HeapScanDesc scan, ScanDirection direction)
{
	if (IsHeapAccessedStream(scan->rs_rd->rd_rel->relkind)) /* @BdtcqSK */
	{
	  return streamheap_getnext(scan, direction);
	}							/* @EdtcqSK */
		   ...
		   PgStat_Info *pgstat_info)
{
	...
	if (IsHeapAccessedStream(relation->rd_rel->relkind))
		elog(ERROR, "heap_fetch is not supported on streamheaps");
...
heap_insert(Relation relation, HeapTuple tup, CommandId cid)
{
	...
	//	fprintf(stderr, "DEBUG: heap_insert called...\n");

	if (IsHeapAccessedStream(relation->rd_rel->relkind))	/* @BdtcqSK */
	{
		return streamheap_insert(relation, tup, cid);
	}							/* @EdtcqSK */
			...
			ItemPointer ctid, CommandId cid)
{
	...
	if (IsHeapAccessedStream(relation->rd_rel->relkind))
		elog(ERROR, "stream access method does not support delete");
	/* increment access statistics */
			...
			ItemPointer ctid, CommandId cid)
{
	...
	if (IsHeapAccessedStream(relation->rd_rel->relkind))
		elog(ERROR, "stream access method does not support update");
	/* increment access statistics */
