 ...
 *
 * IDENTIFICATION
 *	  $Header: /project/eecs/db/cvsroot/postgres/src/backend/access/transam/xact.c,v 1.11 2004/03/24 08:10:50 chungwu Exp $
 *
 * NOTES
...
{
    if (elog_transactions) {
        elog(LOG, "Committing transaction %d.", GetCurrentTransactionId());
    }
	...
	if (MyXactMadeXLogEntry || MyXactMadeTempRelUpdate)
	{
		...
		if (MyLastRecPtr.xrecoff != 0)
		{
            ...
            if (elog_transactions) {
                elog(LOG, "    Writing commit record.");
            }
		...
		{
            if (elog_transactions) {
                elog(LOG, "    NOT writing commit record.");
            }
		...
		if (MyXactMadeXLogEntry)
		{
            ...
            if (elog_transactions) {
                elog(LOG, "    Flushing transaction log.");
            }
        ...
        if (elog_transactions) {
            elog(LOG, "    Not writing anything about this transaction.");
        }
    }
...
{
    if (elog_transactions) {
        elog(LOG, "Aborting transaction %d.", GetCurrentTransactionId());
    }
...
StartTransaction(void)
{
	...
	if (s->state == TRANS_INPROGRESS) {
        if (elog_transactions) {
            elog(LOG, "Continuing transaction %d.", s->transactionIdData);
        }
    ...
    if (elog_transactions) {
        elog(LOG, "Starting transaction %d.", s->transactionIdData);
    }
...
AbortTransaction(void)
{
    ...
    if (elog_transactions) {
        elog(LOG, "Aborting transaction %d.", s->transactionIdData);
    }
...
CommitTransactionCommand(bool forceCommit)
{
	...
	switch (s->blockState)
	{
			 ...
			 * PreventTransactionChain call during the transaction. (The
			 * parameters could be omitted, but it turns out most callers
			 * of StartTransactionCommand/CommitTransactionCommand want to
			 * force autocommit, so making them all call
...
AbortCurrentTransaction(void)
{
	...
	if (UseTelegraphCQ && cqcancelid >= 0)
	{
		Controlqentry cqe;

		elog(DEBUG1, "abort current transaction is notifying the tcq bacend of cancellation for query %d",
			 cqcancelid);
		cqe.command = QUERY_REMOVE;
		cqe.data.queryid = cqcancelid;
