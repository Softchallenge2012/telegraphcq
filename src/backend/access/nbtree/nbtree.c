 ...
 *
 * IDENTIFICATION
 *	  $Header: /project/eecs/db/cvsroot/postgres/src/backend/access/nbtree/nbtree.c,v 1.3 2004/03/24 08:10:48 chungwu Exp $
 *
 *-------------------------------------------------------------------------
 */
...
btbulkdelete(PG_FUNCTION_ARGS)
{
	 ...
	 *
	 * Whenever we step onto a new page, we have to trade in the read lock
	 * acquired by _bt_first or _bt_step for an exclusive write lock
	 ...
	 * those containing items we need to delete, but unfortunately that is
	 * not safe: we could then pass a stopped indexscan, which could in
	 * rare cases lead to deleting the item it needs to find when it
	 * resumes.  (See _bt_restscan --- this could only happen if an
	 * indexscan stops on a deletable item and then a page split moves
	 * that item into a page further to its right, which the indexscan
	 * will have no pin on.)
	 */
	...
	if (_bt_first(scan, ForwardScanDirection))
	{
		...
		do
		{
			...
			/*
			 * Make sure we have a super-exclusive write lock on this
			 * page.
			 *
			 * We assume that only concurrent insertions, not deletions, can
			 * occur while we're not holding the page lock (the caller
			 * should hold a suitable relation lock to ensure this).
			 * Therefore, no items can escape being scanned because of
			 * this temporary lock release.
			 */
			if (blkno != lockedBlock)
			{
				...
				/*
				 * If the page was formerly rightmost but was split while
				 * we didn't hold the lock, and ip_posid is pointing to
				 * item 1, then ip_posid now points at the high key not a
				 * valid data item. In this case we need to step forward.
				 */
			...
			if (callback(htup, callback_state))
			{
				...
				/*
				 * We now need to back up the scan one item, so that the
				 * next cycle will re-examine the same offnum on this page
...
_bt_restscan(IndexScanDesc scan)
{
	...
	for (;;)
	{
		 ...
		 * did, it might be able to reach and delete our target item
		 * before we can find it again.
		 */
