 ...
 *
 * IDENTIFICATION
 *	  $Header: /project/eecs/db/cvsroot/postgres/src/backend/access/common/printtup.c,v 1.9 2004/07/15 20:39:53 owenc Exp $
 *
 *-------------------------------------------------------------------------
 */
#include "postgres.h"
...
printtup(HeapTuple tuple, TupleDesc typeinfo, DestReceiver *self)
{
	...
	/* If the tuple is NULL, then the current window is dry, so send
	 * a "window dry" message to the client
	 */
	if (tuple == NULL)
	{
	  pq_beginmessage(&buf);
...
static void
f_printatt(unsigned attributeId,
		   Form_pg_attribute attributeP,
		   char *value,
		   FILE *fp)
{
	fprintf(fp, "\t%2d: %s%s%s%s\t(typeid = %u, len = %d, typmod = %d, byval = %c)\n",
			attributeId,
...
printatt(unsigned attributeId,
		 ...
		 char *value)
{
	f_printatt(attributeId, attributeP, value, stdout);
}
 ...
 */
static void
printatt_lev(unsigned attributeId,
			   Form_pg_attribute attributeP,
			   char *value,int lev)
{
	elog(lev, "\t%2d: %s%s%s%s\t(typeid = %u, len = %d, typmod = %d, byval = %c)",
		 attributeId,
		 NameStr(attributeP->attname),
		 value != NULL ? " = \"" : "",
		 value != NULL ? value : "",
 ...
 */
static void
printatt_debug(unsigned attributeId,
			   Form_pg_attribute attributeP,
			   char *value)
{
	elog(DEBUG1, "\t%2d: %s%s%s%s\t(typeid = %u, len = %d, typmod = %d, byval = %c)",
		 attributeId,
		 NameStr(attributeP->attname),
		 value != NULL ? " = \"" : "",
		 value != NULL ? value : "",
...
{
	f_debugtup(tuple, typeinfo, self, stdout);
}

void
f_debugtup(HeapTuple tuple, TupleDesc typeinfo, DestReceiver *self, FILE *fp)
{
	...
	for (i = 0; i < natts; ++i)
	{
							  ...
							  &typoutput, &typelem, &typisvarlena))
		{
			...
			f_printatt((unsigned) i + 1, typeinfo->attrs[i], value, fp);
...
debugtup_debug(HeapTuple tuple, TupleDesc typeinfo, DestReceiver *self)
{
	...
	for (i = 0; i < natts; ++i)
	{
							  ...
							  &typoutput, &typelem, &typisvarlena))
		{
			...
			printatt_debug((unsigned) i + 1, typeinfo->attrs[i], value);
...
void
debugtup_lev(HeapTuple tuple, TupleDesc typeinfo, DestReceiver *self,
	     int lev)
{
	int			natts = tuple->t_data->t_natts;
	int			i;
	...
	for (i = 0; i < natts; ++i)
	{
							  ...
							  &typoutput, &typelem, &typisvarlena))
		{
			...
			printatt_lev((unsigned) i + 1, typeinfo->attrs[i], value, lev);
