 ...
 *
 * IDENTIFICATION
 *	  $Header: /project/eecs/db/cvsroot/postgres/src/backend/access/common/tupdesc.c,v 1.14 2005/02/09 00:16:40 phred Exp $
 *
 * NOTES
...
CreateTupleDescCopyConstr(TupleDesc tupdesc)
{
	...
	if (constr)
	{
        ...
        /* Copy over the (TelegraphCQ) timestamp attribute number. */
        cpy->ts_attrnum = constr->ts_attrnum;
...
BuildDescForRelation(List *schema)
{
	...
	foreach(p, schema)
	{
		...
		if (entry->is_timestamp)	/* @BdtstampSK */
		{
			if (tc_has_timestamp(constr))
				elog(ERROR, "Only one TIMESTAMPCOLUMN constraint allowed");
			constr->ts_attrnum = attnum - 1;
		}						/* @EdtstampSK */
	...
	if (constr->has_not_null || tc_has_timestamp(constr) || ndef > 0)	/* @dtstampSK */
	{
		desc->constr = constr;
