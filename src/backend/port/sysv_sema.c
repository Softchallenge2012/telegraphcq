 ...
 *
 * IDENTIFICATION
 *	  $Header: /project/eecs/db/cvsroot/postgres/src/backend/port/sysv_sema.c,v 1.7 2004/03/24 08:11:04 chungwu Exp $
 *
 *-------------------------------------------------------------------------
 */
#include "postgres.h"
...
static IpcSemaphoreKey nextSemaKey = -1;		/* next key to try using */
...
static IpcSemaphoreId TCQIpcSemaphoreCreate(int numSems);
...
void
TCQSemaphoreCreate(TCQSemaphore sema)
{
	sema->semId = TCQIpcSemaphoreCreate(1);
	sema->semNum = 1;
}
 ...
 */
static IpcSemaphoreId
TCQIpcSemaphoreCreate(int numSems)
{
	IpcSemaphoreId semId;
	union semun semun;
	TCQSemaphoreData sema;

	if (nextSemaKey < 0)
		nextSemaKey = PostPortNumber + 1000;
	...
	for (nextSemaKey++;; nextSemaKey++)
	{
		 ...
		 * quietly.
		 */
		elog(DEBUG1, "reaping a semaphore");
		semun.val = 0;			/* unused, but keep compiler quiet */
		if (semctl(semId, 0, IPC_RMID, semun) < 0)
			continue;
	...
	sema.semId = semId;
	sema.semNum = numSems;
	TCQSemaphoreUnlock(&sema);

	return semId;
}

void
TCQSemaphoreReset(TCQSemaphore sema)
{
	IpcSemaphoreInitialize(sema->semId, sema->semNum, 0);
}

void
TCQSemaphoreLock(TCQSemaphore sema, bool interruptOK)
{
	int			errStatus;
	struct sembuf sops;
...
void
TCQSemaphoreUnlock(TCQSemaphore sema)
{
	int			errStatus;
	struct sembuf sops;
...
bool
TCQSemaphoreTryLock(TCQSemaphore sema)
{
	int			errStatus;
	struct sembuf sops;
 ...
 /* Initialize the value of a semaphore */	/* @BdtcqSK */
void
TCQSemaphoreInitialize(TCQSemaphore sema, int value)
{
	IpcSemaphoreInitialize(sema->semId, sema->semNum, value);
}

/* Get the current value (semval) of the semaphore */
int
TCQSemaphoreGetValue(TCQSemaphore sema)
{
	return IpcSemaphoreGetValue(sema->semId, sema->semNum);
}

/* Removes a semaphore set */
void
TCQSemaphoreKill(TCQSemaphore sema)
{
...
PGSemaphoreCreate(PGSemaphore sema)
{
	...
	if (IsUnderPostmaster)
		elog(FATAL, "PGSemaphoreCreate cannot be called except in the postmaster");
