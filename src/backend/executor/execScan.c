 ...
 *
 * IDENTIFICATION
 *	  $Header: /project/eecs/db/cvsroot/postgres/src/backend/executor/execScan.c,v 1.15 2004/03/24 08:10:54 chungwu Exp $
 *
 *-------------------------------------------------------------------------
 */
#include "postgres.h"
		 ...
		 ExecScanAccessMtd accessMtd)	/* function returning a tuple */
{
	...
	qual = node->plan.qual;
	dontProject = node->dontProject;	/* @deddySK */
	...
	for (;;)
	{
		...
		slot = (*accessMtd) (node);
		if (slot)
			slot->ttc_isIntermediateTuple = false;
		...
		/** Found a tuple -- update stats @dstemsAD */
		node->num_tuples_seen_so_far++;
		...
		if (!qual || ExecQual(qual, econtext, false))
		{
			...
			/** Found a satisfied tuple -- update stats @dstemsAD */
			node->num_tuples_passed_quals_so_far++;

			if (!dontProject)	/* @BdeddySK */
			{
				resultSlot = ExecProject(scanstate->cstate.cs_ProjInfo, &isDone);
        ...
        {
            if(node->return_null_if_quals_fail)
            {
                return ExecStoreTuple(NULL,
                        scanstate->cstate.cs_ProjInfo->pi_slot,
                        InvalidBuffer,
