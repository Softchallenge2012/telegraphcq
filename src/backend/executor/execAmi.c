 ...
 *
 *	$Id: execAmi.c,v 1.10 2004/08/25 06:57:37 owenc Exp $
 *
 *-------------------------------------------------------------------------
 */
#include "postgres.h"
...
ExecReScan(Plan *node, ExprContext *exprCtxt, Plan *parent)
{
	...
	case T_FAgg:
	  ExecReScanFAgg((FAgg*) node, exprCtxt, parent);
	  break;
		...
		case T_Group:
			...
			ExecReScanWindow((Window *) node, exprCtxt, parent);
#else
			Assert(!"Calling CQ code when CQ Support is disabled");
#endif
			break;
			@EwndaggMS */
		...
		case T_Append:
			...
			break;
		case T_StrmScan:
			ExecReScanStrmScan((StrmScan *) node, exprCtxt, parent);
			break;
		default:
