 ...
 *
 * IDENTIFICATION
 *	  $Header: /project/eecs/db/cvsroot/postgres/src/backend/executor/nodeAgg.c,v 1.13 2006/02/06 01:19:46 phred Exp $
 *
 *-------------------------------------------------------------------------
 */
...
bool AggConsumeNextInput(Agg *node);
...
/* FUNCTION AggConsumeNextInput
 * Processes the next input tuple from the outerPlan of the Agg node.  Returns
 * TRUE if it found a tuple; FALSE otherwise.
 */
bool AggConsumeNextInput(Agg *node) 
{
	AggState   *aggstate;
    ...
    if (TupIsNull(outerslot)) {
        /*elog(LOG, "nodeAgg: Got a null tuple!");*/
        return false;
    }

    /* Skip over punctuation tuples without putting them through the
     * aggregate. */
    if (!(outerslot->ttc_isIntermediateTuple)
        && aggstate->ignorePunct && HeapTupleIsPunct(outerslot->val)) 
    {
        elog(DEBUG1, "nodeAgg: Ignoring a punctuation tuple.");
        /* TODO: Do we need to deallocate the tuple? */
        return true;
    } 

    /*
      elog(NOTICE,"AggGotATuple");
      debugtup_lev(outerslot->val,
               outerslot->ttc_tupleDescriptor,NULL, NOTICE);
    */
     ...
     * aggregated over.)
     */
    if (!aggstate->inputTuple)
        aggstate->inputTuple = heap_copytuple(outerslot->val);
...
ExecAgg(Agg *node)
{
	...
	do
	{
		...
		aggstate->inputTuple = NULL;		/* no saved input tuple yet */
		...
		for (;;)
		{
            bool result = AggConsumeNextInput(node);
		...
		/*
		 * If the outerPlan is a Group or Window node, we will reach here
		 * after each group or window.	We are not done unless the
		 * Group/Window node is done (a little ugliness here while we
		 * reach into the Group/Window's state to find out). Furthermore,
		 * when grouping we return nothing at all unless we had some input
		 * tuple(s).  By the nature of Group/Window, there are no empty
		 * groups, so if we get here with no input the whole scan is
		 * empty.
		 *
		 * If the outerPlan isn't a Group or Window, we are done when we get
		 * here, and we will emit a (single) tuple even if there were no
		 * input tuples.
		 */
		if (IsA(outerPlan, Group))
		{
			...
			/* check for no groups */
			if (aggstate->inputTuple == NULL)
				return NULL;
		}
		else if (IsA(outerPlan, Window))		/* @BwndaggWH */
		{
			/* aggregation over windows */
            elog(FATAL, "Should not be checking for window done!");
            /*
		...
		else
		{
			...
			/*
			 * If aggstate->inputtuple==NULL (ie, the outerPlan didn't return
			...
			if (aggstate->inputTuple == NULL)
			{
				...
				if (tupType && tupType->natts > 0)
				{
					...
					aggstate->inputTuple = heap_formtuple(tupType, dvalues, dnulls);
					pfree(dvalues);
					pfree(dnulls);
				}
			}
		 ...
		 * from the slot.
		 */
		ExecStoreTuple(aggstate->inputTuple,
...
#define AGG_NSLOTS 2
	...
	{
		ExecInitNewResultTupleSlot(estate, &aggstate->csstate.cstate);
		ExecInitNewScanTupleSlot(estate, &aggstate->csstate);
		InitSlotForEddy(aggstate->csstate.cstate.cs_ResultTupleSlot);
		InitSlotForEddy(aggstate->csstate.css_ScanTupleSlot);
	}
...
ExecEndAgg(Agg *node)
{
	...
	ExecFreeSlotIfOwned(aggstate->csstate.cstate.cs_ResultTupleSlot);
	ExecFreeSlotIfOwned(aggstate->csstate.css_ScanTupleSlot);
}
...
ExecReScanAgg(Agg *node, ExprContext *exprCtxt, Plan *parent)
{
	...
	/*bool insideFAgg = (MyProcessType == PROC_TCQ_BACKEND);*/
	aggstate->agg_done = false;
