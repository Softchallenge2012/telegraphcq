 ...
 *
 * IDENTIFICATION
 *	  $Header: /project/eecs/db/cvsroot/postgres/src/backend/executor/execQual.c,v 1.23 2005/05/04 02:28:37 phred Exp $
 *
 *-------------------------------------------------------------------------
 */
/*
...
static Datum
ExecEvalVarBranch(AttrNumber attnum, Oid vartype,		/* @dstemsAD */
				HeapTuple heapTuple, TupleDesc tuple_type, bool *isNull);		/* @dstemsAD */
...
ExecEvalVar(Var *variable, ExprContext *econtext, bool *isNull)
{
	...
	/*
	 * printf("econtext %x %x %x\n", econtext->ecxt_scantuple,
	 * econtext->ecxt_innertuple, econtext->ecxt_outertuple);
	 */

	/*
	 * printf("vals %x %x %x\n", econtext->ecxt_innertuple ?
	 * econtext->ecxt_innertuple->val : 0,
	 */
	/* econtext->ecxt_outertuple ? econtext->ecxt_outertuple->val : 0,	*/
	/* econtext->ecxt_scantuple ? econtext->ecxt_scantuple->val : 0);  */
	/* nodeDisplay(variable); */
	...
	/** If intermediate representation, need to do different things. *//* @
	 * BdstemsAD */
	if (!slot->ttc_isIntermediateTuple)
		return ExecEvalVarBranch(variable->varattno, variable->vartype, slot->val, slot->ttc_tupleDescriptor, isNull);
	else
	{
		/*
		 * We need to use varnoold - 1 to get to the corresponding base
		 * tuple, since the postgres numbers relations in the query
		 * starting with 1
		 */
		IntermediateHeapTuple iht = (IntermediateHeapTuple) slot->val;

#ifndef DISABLE_CQ_SUPPORT
		TupleDesc	td_temp = iht->t_base_tuple_descs[variable->varnoold - 1];
		td = iht->t_base_tuple_descs[variable->varnoold - 1];
#else
		td = slot->ttc_base_tuple_descs_ptr[variable->varnoold - 1];
#endif
		ht = iht->t_base_tuples[variable->varnoold - 1];

#ifndef DISABLE_CQ_SUPPORT
		Assert(td == td_temp);
#endif

		/** There are slight differences based on whether or not fast path was used. XXX */
		if (!UseFastPathEddyCreate)
			return ExecEvalVarBranch(variable->varoattno, variable->vartype, ht, td, isNull);
		else
			return ExecEvalVarBranch(variable->varattno, variable->vartype, ht, td, isNull);
	}							/* @EdstemsAD */
}

static Datum					/* @BdstemsAD */
ExecEvalVarBranch(AttrNumber attnum, Oid vartype, HeapTuple heapTuple, TupleDesc tuple_type, bool *isNull)
{
	Datum		result;
	...
	Assert(attnum <= 0 ||
		   (attnum - 1 <= tuple_type->natts - 1 &&
			...
			vartype == tuple_type->attrs[attnum - 1]->atttypid));
...
Datum
ExecMakeFunctionResultSwitchContext(FunctionCachePtr fcache,
									List *arguments,
									ExprContext *econtext,
									bool *isNull,
									ExprDoneCond *isDone)
{
	Datum		retDatum = PointerGetDatum(NULL);
	MemoryContext oldContext = NULL;

	if (econtext != NULL)
		oldContext = MemoryContextSwitchTo(econtext->ecxt_per_tuple_memory);
	retDatum = ExecMakeFunctionResult(fcache, arguments, econtext, isNull, isDone);
	if (oldContext != NULL)
		MemoryContextSwitchTo(oldContext);
	return retDatum;
...
#define NPREALLOCDOMAINS 64
    ...
    /* HACK ALERT: Propogate the HEAP_ISPUNCT bit to the new HeapTuple. */
    if (NULL != econtext
            && NULL != econtext->ecxt_scantuple
            && NULL != econtext->ecxt_scantuple->val)
    {
        TupleTableSlot *slot = econtext->ecxt_scantuple;
        bool ispunct = false;
        if (econtext->ecxt_scantuple->ttc_isIntermediateTuple) {
            ispunct = ihtIsPunct( (IntermediateHeapTuple) slot->val);
        } else {
            ispunct = HeapTupleIsPunct(slot->val);
        }

        if (ispunct) {
            newTuple->t_data->t_infomask |= HEAP_ISPUNCT;
        }
    }
