 ...
 *
 * IDENTIFICATION
 *	  $Header: /project/eecs/db/cvsroot/postgres/src/backend/executor/nodeSort.c,v 1.7 2006/02/06 01:19:46 phred Exp $
 *
 *-------------------------------------------------------------------------
 */
...
/* Enable debugging output for this file. */
/*#define EXEC_SORTDEBUG 1*/
	...
	if (!sortstate->sort_Done)
	{
		...
		for (;;)
		{
			...
			if (TupIsNull(slot))
			{
                SO1_printf("ExecSort(%p): %s", node, 
                        "Got a null tuple");
				if (IsA(outerPlan(node), Window))		/* @BdwndaggSK */
				{
                    elog(FATAL, "Should never be reached");
                    /* TODO: Should the following be commented out?
					Window	   *outerwin = (Window *) outerPlan(node);

					sortstate->child_Done = outerwin->wndstate->wnd_done;
                    */
				}
				else if (IsA(outerPlan(node), Agg)) {
					sortstate->child_Done = 
                        ((Agg *) outerPlan(node))->aggstate->agg_done;
                }
				else
				{
					sortstate->child_Done = true;
				}				/* @EdwndaggSK */
				break;
			}
...
ExecInitSort(Sort *node, EState *estate, Plan *parent)
{
	...
	SO1_printf("ExecInitSort(%p): %s", node,
	...
	sortstate->sort_Done = false;
	sortstate->child_Done = false;		/* @dtcqSK */
	sortstate->tuplesortstate = NULL;
...
#define SORT_NSLOTS 2
	...
	{
		ExecInitNewResultTupleSlot(estate, &sortstate->csstate.cstate);
		ExecInitNewScanTupleSlot(estate, &sortstate->csstate);
		InitSlotForEddy(sortstate->csstate.cstate.cs_ResultTupleSlot);
		InitSlotForEddy(sortstate->csstate.css_ScanTupleSlot);
	}	
...
ExecEndSort(Sort *node)
{
	 ...
	 */
	SO1_printf("ExecEndSort(%p): %s", node,
	...
	ExecFreeSlotIfOwned(sortstate->csstate.cstate.cs_ResultTupleSlot);
	ExecFreeSlotIfOwned(sortstate->csstate.css_ScanTupleSlot);
	/*
	 * Release tuplesort resources
	 */
	if (sortstate->tuplesortstate != NULL)
...
ExecReScanSort(Sort *node, ExprContext *exprCtxt, Plan *parent)
{
	...
	if ( (((Plan *) node)->lefttree->chgParam != NULL) ||
            ((Plan*)node)->insideFAgg)
	{
        /*elog(LOG,"sort %p calling rescan on its child", node);*/
		sortstate->sort_Done = false;
		tuplesort_end((Tuplesortstate *) sortstate->tuplesortstate);
		sortstate->tuplesortstate = NULL;
