 ...
 *
 * IDENTIFICATION
 *	  $Header: /project/eecs/db/cvsroot/postgres/src/backend/executor/nodeLimit.c,v 1.3 2006/02/06 01:19:46 phred Exp $
 *
 *-------------------------------------------------------------------------
 */
/*
...
#define LIMIT_NSLOTS 1
    ...
    if (ExecUseNewSlots((Plan *) node, parent)) {
        /* Use TelegraphCQ-style tuple table management. */
        ExecInitNewResultTupleSlot(estate, &limitstate->cstate);
        InitSlotForEddy(limitstate->cstate.cs_ResultTupleSlot);

    } else {
        /* Non-TelegraphCQ mode. */
