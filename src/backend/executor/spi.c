 ...
 *
 * IDENTIFICATION
 *	  $Header: /project/eecs/db/cvsroot/postgres/src/backend/executor/spi.c,v 1.8 2004/03/24 08:10:56 chungwu Exp $
 *
 *-------------------------------------------------------------------------
 */
#include "postgres.h"
...
_SPI_execute(char *src, int tcount, _SPI_plan *plan)
{
	...
	foreach(list_item, raw_parsetree_list)
	{
		...
		foreach(query_list_item, query_list)
		{
			...
			/* TBD TBD TBD - Blecch .. check 2nd arg to pg_plan_query */
			planTree = pg_plan_query(queryTree, CurrentMemoryContext);
