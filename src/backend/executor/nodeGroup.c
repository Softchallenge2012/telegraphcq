 ...
 *
 * IDENTIFICATION
 *	  $Header: /project/eecs/db/cvsroot/postgres/src/backend/executor/nodeGroup.c,v 1.10 2006/02/06 01:19:46 phred Exp $
 *
 *-------------------------------------------------------------------------
 */
...
ExecGroupEveryTuple(Group *node)
{
	...
	else
	{
		...
		{
			if (IsA(outerPlan(node), Sort))		/* @BdwndaggSK */
			{
				/*
				 * Sort node can be below Group. The CQ semantics are that
				 * it's not done until the sortstate says so.
				 */
				grpstate->grp_done = ((Sort *) outerPlan(node))->sortstate->child_Done;
...
#define GROUP_NSLOTS 2
	...
	{
		ExecInitNewResultTupleSlot(estate, &grpstate->csstate.cstate);
		ExecInitNewScanTupleSlot(estate, &grpstate->csstate);
		InitSlotForEddy(grpstate->csstate.cstate.cs_ResultTupleSlot);
		InitSlotForEddy(grpstate->csstate.css_ScanTupleSlot);
	}	
	/*
...
ExecEndGroup(Group *node)
{
	...
	ExecFreeSlotIfOwned(grpstate->csstate.cstate.cs_ResultTupleSlot);
	ExecFreeSlotIfOwned(grpstate->csstate.css_ScanTupleSlot);
	if (grpstate->grp_firstTuple != NULL)
	{
		heap_freetuple(grpstate->grp_firstTuple);
		grpstate->grp_firstTuple = NULL;
