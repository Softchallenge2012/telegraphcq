 ...
 *
 * IDENTIFICATION
 *	  $Header: /project/eecs/db/cvsroot/postgres/src/backend/executor/execTuples.c,v 1.13 2004/07/15 20:39:54 owenc Exp $
 *
 *-------------------------------------------------------------------------
 */
/*
...
ExecAllocTableSlot(TupleTable table)
{
	...
	if(MyProcessType == PROC_TCQ_BACKEND &&
	   UseCQEddy)
	  {
	    elog(FATAL, "TCQ backend trying to allocate a tuple table slot.  This shouldn't happen");
	  }
	...
	slot->inTupleTable = TRUE;

	return slot;
}

/*
 *	ExecUseNewSlots -- should the named operator &parent pair allocate
 *	  tuple table slots from the executors tuple table, or use
 *	  freestanding slots.  The operator can use this function to
 *	  decide if it should call ExecAllocSlot or MakeTupleTableSlot.
 *	  if freshly allocated slots are used, then the inTupleTable
 *	  should be set to false in the slot so it can be freed later
 *	  with ExecFreeSlotIfOwned.
 */


bool
ExecUseNewSlots(Plan *node, Plan *parent)
{

#ifdef DISABLE_CQ_SUPPORT
	Assert(!"CQ Stuff used while CQ support disabled");
#endif

	if(MyProcessType == PROC_TCQ_BACKEND && UseCQEddy)
	  return TRUE;
	/*
	 * changed because FAggregate plans must use allocated slots 
	    if (parent == NULL)
	    return FALSE;
	    return ((IsA(parent, Eddy) || IsA_Fjord(parent)) &&
	    (IsA(node, SeqScan) ||IsA(node, ScanQueue) || IsA(node, SteM) ||
	    IsA(node, StrmScan)));
	*/
	return FALSE;
}
...
void
InitSlotForEddy(TupleTableSlot *tts)
{
	tts->ttc_shouldFreeDesc = false;
	tts->ttc_shouldFree = false;

}

void
ExecFreeSlotIfOwned(TupleTableSlot *tts)
{
	/* must drop descriptor, as this is only normally done when  */
	/* assigning a new descriptor to a slot, or when freeing the  */
	/* entire tuple table. */

	if (tts == NULL)
	{
		elog(NOTICE, "ExecFreeSlotIfOwned called with null argument");
		return;
	}

	if (tts->inTupleTable == FALSE)
	{
		ExecClearTuple(tts);
...
#ifdef DISABLE_CQ_SUPPORT
    /* Assert(!"This function does not init the base_tuples_desc so for the moment, it is disabled"); XXX */
#endif
 ...
 */
void
ExecInitNewResultTupleSlot(EState *estate, CommonState *commonstate)
{
	commonstate->cs_ResultTupleSlot = MakeTupleTableSlot();
 ...
 */
void
ExecInitNewScanTupleSlot(EState *estate, CommonScanState *commonscanstate)
{
	commonscanstate->css_ScanTupleSlot = MakeTupleTableSlot();
...
AttOutMetadata *
TupleDescGetAttOutMetadata(TupleDesc tupdesc)
{
	int			natts = tupdesc->natts;
	int			i;
	Oid			atttypeid;
	Oid			attoutfuncid;
	FmgrInfo   *attoutfuncs;
	Oid		   *attelems;
	bool	   *attisvarlena;
	AttOutMetadata *attoutmeta;

	attoutmeta = (AttOutMetadata *) palloc(sizeof(AttOutMetadata));

	/*
	 * Gather info needed later to call the "in" function for each
	 * attribute
	 */
	attoutfuncs = (FmgrInfo *) palloc(natts * sizeof(FmgrInfo));
	attelems = (Oid *) palloc(natts * sizeof(Oid));
	attisvarlena = (bool *) palloc(natts * sizeof(bool));


	for (i = 0; i < natts; i++)
	{
		atttypeid = tupdesc->attrs[i]->atttypid;
		getTypeOutputInfo(atttypeid, &attoutfuncid, &attelems[i],
						  &attisvarlena[i]);
		fmgr_info(attoutfuncid, &attoutfuncs[i]);
	}
	attoutmeta->tupdesc = tupdesc;
	attoutmeta->attoutfuncs = attoutfuncs;
	attoutmeta->attelems = attelems;
