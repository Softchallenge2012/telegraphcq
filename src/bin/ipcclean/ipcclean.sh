#!/bin/sh
#
# $Header: /project/eecs/db/cvsroot/postgres/src/bin/ipcclean/ipcclean.sh,v 1.3 2005/06/13 02:59:28 phred Exp $
#
...
if [ `uname` = 'Linux' ]; then
    ...
    # shared memory
    for val in `ipcs -m -p | grep $USER | grep '^[0-9]' | awk '{printf "%s:%s:%s\n", $1, $3, $4}'`
    do
	save_IFS=$IFS
	IFS=:
	set X $val
