 ...
 *
 * $Header: /project/eecs/db/cvsroot/postgres/src/bin/psql/startup.c,v 1.8 2004/03/24 08:11:26 chungwu Exp $
 */
...
main(int argc, char *argv[])
{
	...
	do
	{
		...
		pset.db = PQsetdbLogin(options.host, options.port, NULL, NULL,
							   options.action ==
							   ACT_LIST_DB ? "template1" : options.dbname,
							   username, password);

		if (PQstatus(pset.db) == CONNECTION_BAD &&
			strcmp(PQerrorMessage(pset.db),
	...
	else if (options.action == ACT_SINGLE_SLASH)
	{
		...
		if ((value = GetVariable(pset.vars, "ECHO"))
			&& strcmp(value, "all") == 0)
	...
	else
	{
		...
		if (!QUIET() && !pset.notty)
		{
			...
			printSSLInfo();
#endif
			if (GetVariable(pset.vars, "CQ") != NULL)
			{
				printf
...
parse_psql_options(int argc, char *argv[], struct adhoc_opts * options)
{
	...
	static struct option long_options[] = {
		...
		{"rows-per-iteration", required_argument, NULL, 'i'},
		{"rows-per-query", required_argument, NULL, 'I'},
	...
	while ((c =
			getopt_long(argc, argv,
						"aAc:Cd:eEf:F:h:Hi:I:lno:p:P:qR:sStT:uU:v:VWxX?",
	...
	while ((c =
			getopt(argc, argv,
			   "aAc:Cd:eEf:F:h:Hi:I:lno:p:P:qR:sStT:uU:v:VWxX?-")) != -1)
#endif   /* not HAVE_GETOPT_LONG */
	{
		switch (c)
		{
			...
			case 'i':
				...
				/*
				 * TODO: Should probably complain if this option is set
				 * when -C is not set.
				 */
				pset.cq_rows_iter = atoi(optarg);
				fprintf(stderr,
						"%d rows per iteration.\n", pset.cq_rows_iter);
				break;
			case 'I':
