 ...
 *
 * $Header: /project/eecs/db/cvsroot/postgres/src/bin/psql/print.c,v 1.6 2004/11/23 22:14:53 phred Exp $
 */
                   ...
                   unsigned int *ext_widths)
{
    ...
    /* The final carriage return puts a line between query results in CQ mode, 
     * so we omit it to make parsing them easier. */
    if (!opt_cq_mode) {
        fputc('\n', fout);
    }
...
print_aligned_vertical(const char *title, const char *const * headers,
				  const char *const * cells, const char *const * footers,
					   bool opt_barebones, bool opt_cq_mode, 
                       unsigned short int opt_border,
					   FILE *fout)
{
	unsigned int col_count = 0;
           ...
           unsigned int *widths)
{
	...
	switch (opt->format)
	{
		...
		case PRINT_ALIGNED:
			if (opt->expanded)
				print_aligned_vertical(title, headers, cells, footers, opt->tuples_only, opt->cq_mode, border, output);
			else
				print_aligned_text(title, headers, cells, footers, align, opt->tuples_only, opt->cq_mode, border, output, widths);
			break;
		case PRINT_HTML:
			if (opt->expanded)
