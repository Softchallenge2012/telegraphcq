...
======= end
 *
 * $Header: /project/eecs/db/cvsroot/postgres/src/bin/psql/describe.c,v 1.10 2004/11/23 22:14:53 phred Exp $
 */
...
describeAggregates(const char *pattern, bool verbose)
{
	...
	printQuery(res, &myopt, pset.queryFout, NULL);
						...
						bool verbose)
{
	...
	switch (tableinfo.relkind)
	{
		...
		case 'c':
			...
			break;
		case 'T':
			printfPQExpBuffer(&title, _("Unarchived stream \"%s.%s\""),
							  schemaname, relationname);
			break;
		case 'A':
			printfPQExpBuffer(&title, _("Archived stream \"%s.%s\""),
							  schemaname, relationname);
			break;
		default:
	...
	printTable(title.data, headers,
			   ...
			   "llll", &myopt, pset.queryFout, NULL);
...
listTables(const char *tabtypes, const char *pattern, bool verbose)
{
	...
	bool		showStreams = strchr(tabtypes, 'z') != NULL;

	PQExpBufferData buf;
	PGresult   *res;
	printQueryOpt myopt = pset.popt;

	if (!(showTables || showIndexes || showViews || showSeq || showStreams))
	...
	printfPQExpBuffer(&buf,
					  "SELECT n.nspname as \"%s\",\n"
					  "  c.relname as \"%s\",\n"
					  "  CASE c.relkind WHEN 'r' THEN '%s' WHEN 'v' THEN '%s' WHEN 'i' THEN '%s' WHEN 'S' THEN '%s' WHEN 's' THEN '%s' WHEN 'T' THEN '%s' WHEN 'A' THEN '%s' END as \"%s\",\n"
					  "  u.usename as \"%s\"",
					  _("Schema"), _("Name"),
					  _("table"), _("view"), _("index"), _("sequence"),
	_("special"), _("unarchived stream"), _("archived stream"), _("Type"),
					  _("Owner"));
	...
	if (showSystem && showTables)
		appendPQExpBuffer(&buf, "'s',");
	if (showStreams)			/* @dtcqSK */
		appendPQExpBuffer(&buf, "'T','A',");	/* @dtcqSK */
